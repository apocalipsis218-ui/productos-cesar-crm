...
// V9.3.0 R9 · Usuarios vinculados con empleados y despacho compacto para tablet.
...
// V9.3.0 R9 - despacho compacto para tablet y observaciones por artículo
...
function isCommercialNormalOrder(o){
  return !!o && orderType(o)==='Pedido normal' && norm(o.canal)!=='caso especial';
}
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.0 R9</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.0 R9</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.0 R9</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.0 R9</span></div>
...
function openOrderForm(o=null, client=null, extra={}){
  ...
  const drawItems=(opts={})=>{
    ...
    box.innerHTML=lineItems.length?`<div class="order-list-header"><span>Producto / observación</span><span>Cantidad</span><span>Precio</span><span>Subtotal</span><span></span></div>`+lineItems.map((it,idx)=>{
      const noFrac=it.permite_fraccion===false;
      const note=itemInstruction(it);
      return `<div class="order-row order-row-with-note" data-line="${idx}"><div class="order-main"><b>${esc(it.producto_nombre||'Producto')}</b><small>${it.producto_id?'Catálogo':'No listado'} · ${esc(it.unidad||'lb')} · ${money(it.precio||0)}${noFrac?' · No granel':''}</small><input class="order-item-note-inline" data-row-note="${idx}" maxlength="180" value="${esc(note)}" placeholder="Corte / observación del artículo"></div><input data-row-qty="${idx}" type="number" step="${noFrac?'1':'0.01'}" value="${it.cantidad_pedida||0}" title="Cantidad"><input data-row-price="${idx}" type="number" step="0.01" value="${it.precio||0}" title="Precio"><div class="order-subtotal" data-row-sub="${idx}">${money(it.subtotal||0)}</div><button type="button" class="btn small danger" data-row-del="${idx}">Quitar</button></div>`;
...
function renderValidacion(c){
  ...
  if(state.validacionTab==='historial') renderValidationHistory($('#validationBody',c)); else renderValidationPending($('#validationBody',c));
}
...
function printDeliveryBatchSheet(deliveryName,lote,items,auto=true,opts={}){
  ...
  const rows=items.map(x=>`<tr><td>${esc(x.o.codigo||'')}</td><td>${esc(x.o.cliente?.negocio||'')}</td><td>${esc(x.o.cliente?.telefono||'')}</td><td>${esc(x.o.cliente?.sector||x.o.cliente?.direccion||'')}</td><td>${esc(x.o.factura_no||'—')}</td><td>${money(x.amount)}</td><td>${x.expected?Number(x.expected).toFixed(2)+' lb':'No pesa'}</td><td>${x.peso?Number(x.peso).toFixed(2)+' lb':'—'}</td><td></td></tr>`).join('');
...
function updatePrepWeightUi(m){
  ...
  $$('[data-detail-id]',m).forEach(row=>{
    ...
    if(diffEl){
      ...
      if(req>0 && Math.abs(diff)>tol){ diffEl.textContent=`${diff<0?'Faltan':'Sobran'} ${Math.abs(Number(diff.toFixed(2)))} lb`; diffEl.className=diff<0?'prep-diff peso-bad':'prep-diff peso-warn'; }
      else { diffEl.textContent=''; diffEl.className='prep-diff'; }
    }
  });
}
...
function prepRowsHtml(o, disabled=false){
  ...
  return `<div class="prep-detail-list prep-detail-r8">${items.map(i=>{
    ...
    return `<div class="prep-row prep-row-r8 ${no}" data-detail-id="${i.id}" data-tipo-peso="${esc(tipo)}" data-peso-std="${std}" data-suma-peso="${suma}" data-tolerancia="${tol}" data-pedido-qty="${Number(i.cantidad_pedida||0)}" data-permite-fraccion="${allowFrac}"><div class="prep-product-line"><b class="prod-name">${esc(i.producto_nombre)}</b>${allowFrac?'':'<span class="prep-mini-flag">Solo entero</span>'}</div><div class="prep-compact-grid"><div class="prep-compact-cell prep-requested"><span>Solicitado</span><strong data-pedido="${Number(i.cantidad_pedida||0)}">${Number(i.cantidad_pedida||0)} ${esc(i.unidad||'')}</strong></div><label class="prep-compact-cell"><span>Preparado</span><div class="prep-input-unit"><input ${d} data-prepqty type="number" step="${allowFrac?'0.01':'1'}" value="${prep===''?'':prep}" placeholder="0"><em>${esc(prepInputLabel(i))}</em></div></label><label class="prep-compact-cell prep-state-cell"><span>Estado</span><select ${d} data-prepstate>${opts.map(x=>`<option ${x===st?'selected':''}>${x}</option>`).join('')}</select></label><small data-prep-diff class="prep-diff"></small></div>${instruction?`<div class="prep-item-note"><b>Obs.</b><span>${esc(instruction)}</span></div>`:''}<div data-substitute-box class="prep-substitute prep-substitute-r8" style="${st==='Sustituido'?'':'display:none'}"><select ${d} data-prepsub>${substituteProductOptions(subSelected)}</select><input ${d} data-prepsubqty type="number" step="0.01" value="${subQty===''?'':subQty}" placeholder="Cantidad sustituta"></div><span data-peso-equiv class="prep-equivalent-meta">${Number(equiv.toFixed(2))} lb</span></div>`;
  }).join('')}</div>`;
}
...
function openPreparacionModal(o){
  ...
  m.classList.add('prep-modal-r8'); const prepCard=$('.modal-card',m); if(prepCard) prepCard.classList.add('prep-modal-card-r8');
