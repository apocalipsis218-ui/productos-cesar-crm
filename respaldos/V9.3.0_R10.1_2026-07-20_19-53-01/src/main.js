...
// V9.3.0 R10 · Crear orden y despacho ultracompactos para tablet.
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.0 R10</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.0 R10</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.0 R10</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.0 R10</span></div>
...
function updatePrepWeightUi(m){
  ...
  $$('[data-detail-id]',m).forEach(row=>{
    ...
    if(diffEl){
      ...
      if(req>0 && Math.abs(diff)>tol){ diffEl.textContent=`${diff<0?'Faltan':'Sobran'} ${Math.abs(Number(diff.toFixed(2)))} lb`; diffEl.className=diff<0?'prep-diff prep-shortage-value-r10 peso-bad':'prep-diff prep-shortage-value-r10 peso-warn'; }
      else { diffEl.textContent='—'; diffEl.className='prep-diff prep-shortage-value-r10 is-ok'; }
    }
  });
}
...
function openPreparacionModal(o){
  ...
  m.classList.add('prep-modal-r8','prep-modal-r10'); const prepCard=$('.modal-card',m); if(prepCard) prepCard.classList.add('prep-modal-card-r8','prep-modal-card-r10');
