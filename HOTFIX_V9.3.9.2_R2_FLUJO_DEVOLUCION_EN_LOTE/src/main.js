...
// V9.3.9.2 R2 · Devolución integrada al cotejo del lote, con monto y peso netos.
// Compatibilidad de auditorías históricas: V9.3.9.2 PWA.
...
async function receiveBatchCxcV9392R2(g,payload){
  ...
  const lot=batchRecordByCode(g.code);
  if(!lot?.id) throw new Error('No se encontró el lote formal. Actualiza la pantalla.');
  const {data,error}=await sb.rpc('recibir_lote_cxc_v9392_r2',{
    p_lote_id:Number(lot.id),
    p_items:payload.items,
    p_recibido_por:payload.receivedBy||currentWorkerName(),
    p_observacion:payload.note||null
...
function openLiquidacionOrdenModal(o){
  ...
  res.onchange=()=>{ if(res.value==='Cobrado') cash.value=total; if(res.value==='No entregado') cash.value=0; if(res.value==='Entregado a crédito'&&Number(cash.value||0)>=total) cash.value=0; paintReturn(); paint(); };
...
function openBatchPartialReturnModal(o,draft,onSave){
  ...
  function paint(){
    try{
      ...
      const v=$('#batchReturnValidation',detailModal); v.className='lock-alert ok'; v.innerHTML=`Devolución válida: <b>${money(value.returnedAmount)}</b> · neto ${money(value.netTotal)} · peso devuelto ${value.returnedWeight.toFixed(2)} lb.`;
      return {...value,netWeight};
    }catch(err){
  ...
  $('#saveBatchReturnDraft',detailModal).onclick=()=>{
    const value=paint(); if(value.error) return alert(value.error);
    onSave({...value,cash:value.netTotal,result:'Devuelto parcial'});
    detailModal.remove();
  };
  paint();
}
...
function openCloseBatchLiquidationModal(deliveryName,g){
  ...
  $$('[data-batch-result]',m).forEach(sel=>sel.onchange=()=>{
    ...
    const detailBtn=$(`[data-batch-partial="${sel.dataset.batchResult}"]`,m);
    ...
    if(detailBtn) detailBtn.hidden=sel.value!=='Devuelto parcial';
    paint();
    if(inp && !inp.disabled) focusAndSelect(inp);
  });
  $$('[data-batch-partial]',m).forEach(detailBtn=>detailBtn.onclick=()=>{
