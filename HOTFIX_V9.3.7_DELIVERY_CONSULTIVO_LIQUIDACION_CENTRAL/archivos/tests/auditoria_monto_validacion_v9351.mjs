...
ok(/^9\.3\.(?:[5-9]|[1-9]\d+)$/.test(pkg.version),'package base V9.3.5 con revisión V9.3.5.1 integrada');
ok((main.includes('V9.3.5.1 PWA')||main.includes('V9.3.6 PWA')||main.includes('V9.3.7 PWA'))&&(pwa.includes("APP_VERSION = 'V9.3.5.1 PWA'")||pwa.includes("APP_VERSION = 'V9.3.6 PWA'")||pwa.includes("APP_VERSION = 'V9.3.7 PWA'")),'versión V9.3.5.1 PWA visible');
...
ok(main.includes("total_factura:finalAmount")&&main.includes('Monto final: ${money(finalAmount)}'),'orden y trazabilidad reciben monto definitivo del lote');
ok(main.includes('id="valMonto"')&&main.includes('Monto final de factura *'),'validación individual incluye monto editable');
ok(main.includes("total_factura:monto")&&main.includes('Monto final confirmado ${money(monto)}'),'validación individual guarda monto definitivo');
