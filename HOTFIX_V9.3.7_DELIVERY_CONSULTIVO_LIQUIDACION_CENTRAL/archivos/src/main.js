...
import { calculateCentralReceipt, consolidateFormalLiquidations, buildPendingDeliveryPanel, deliveryReadOnlyMetrics } from './centralLiquidationV937.js';
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7 PWA</span></div>
...
function operOrderCard(o){
  ...
  return `<div class="client-card op-card ${cls} ${orderTypeClass(o)} ${newOrderClass(o,currentModuleOfOrder(o)||'ordenes')}" data-oper-order="${o.id}" style="grid-template-columns:1fr auto;cursor:pointer"><div><div class="client-title">${esc(o.codigo||('ORD-'+o.id))} · ${esc(orderClientName(o))}</div><div class="client-sub">Creada: ${shortDate(o.fecha)} · Despacho: ${shortDate(dispatchDateOf(o))}${o.hora_despacho?' '+esc(String(o.hora_despacho).slice(0,5)):''} · ${esc(o.vendedor||'')} ${o.delivery_nombre?'· Delivery: '+esc(o.delivery_nombre):''}</div><div class="order-status-line">${newOrderBadge(o,currentModuleOfOrder(o)||'ordenes')}${orderCustomerBadge(o)}${orderDeliveryModeBadge(o)}${orderTypeBadge(o)}${specialCaseBadge(o)}${orderStatusBadgeHtml(o)}${orderTakenByBadge(o)}${scheduleBadge(o)}${totalOrderClockBadge(o)}${currentStageClockBadge(o)}<span class="badge">${money(o.total_factura||o.total_estimado)}</span>${o.prioridad&&o.prioridad!=='Normal'?`<span class="badge bad">${esc(o.prioridad)}</span>`:''}${o.factura_no?`<span class="badge ok">Factura ${esc(o.factura_no)}</span>`:''}${prep?`<span class="badge warn">Peso prep. ${Number(prep.libras||0)} lb</span>`:''}${final?`<span class="badge ok">Peso final ${Number(final.libras||0)} lb</span>`:''}<span class="badge">${items.length} producto(s)</span></div>${o.nota_programacion?`<div class="hint"><b>Programación:</b> ${esc(o.nota_programacion)}</div>`:''}${items.length?`<div class="mini-items"><b>Detalle:</b> ${orderItemsText(o,6)}</div>`:''}</div><div class="card-actions">${orderWhatsAppManualButton(o)?`<button class="btn small green" data-wa-order="${o.id}">WhatsApp</button>`:''}<button class="btn small gray" data-oper-order="${o.id}">Ver</button>${o.estado==='Anulado'?'':`${isSpecialOrder(o)?`<button class="btn small" data-special-case="${o.id}">Gestionar caso</button>`:''}${canEditOrderGeneral(o)?`<button class="btn small gray" data-edit-order="${o.id}">Editar</button>`:''}${canDeleteOrder(o)?`<button class="btn small danger" data-cancel-order="${o.id}">${orderHasProgress(o)?'Anular':'Eliminar'}</button>`:''}`}</div></div>`;
}
...
function bindValidationBatch(container,orders){
  ...
  $('#createDeliveryBatch',container).onclick=async()=>{
    ...
    const fechaOriginal=new Date().toISOString(); const validadoPor=currentWorkerName(); const snapshot=buildDeliveryRouteSnapshot(lote,del,val.selected,fechaOriginal,validadoPor);
    const savedLot=await saveFormalDeliveryBatch(lote,del,val.selected,{originalDate:fechaOriginal,validatedBy:validadoPor,snapshot});
    if(savedLot) state.entregaLotes=[savedLot,...(state.entregaLotes||[])];
...
function printValidationRouteFromLot(lot,auto=true){
  const snap=normalizeRouteSnapshot(lot?.hoja_ruta_snapshot); const items=validationBatchRouteItems(lot); if(!items.length) return alert('Este lote no tiene detalle disponible para reimprimir.');
...
function deliveryPendingPanelHtml(rows,selected=''){
  ...
  const all=`<button class="delivery-pending-card-v937 all ${!selected?'selected':''}" type="button" data-liq-delivery-panel=""><b>Todos los deliverys</b><span>${allLots} viaje(s) · ${allClients} cliente(s)</span><strong>${money(allAmount)}</strong></button>`;
...
function bindLiquidacionActionButtons(c, filter, orders, groups){
  ...
  if(printBtn) printBtn.onclick=safe(()=>printLiquidationSummary(filter||'Todos los deliverys',orders));
