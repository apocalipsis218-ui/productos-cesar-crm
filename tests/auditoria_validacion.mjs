...
assert.ok(validationUi.includes('bindValidationBatch(c,orders);'), 'Falta enlazar la barra de lote.');
assert.ok(validationUi.includes('bindDynamic();'), 'Falta enlazar las acciones generales de las órdenes.');

for (const selector of ['data-oper-order','data-return-invoice','data-validate-order']) {
  assert.ok((validationUi+row).includes(selector), `No aparece ${selector} en Validación.`);
...
for (const id of ['selectAllBatch','clearBatch','previewBatchRoute','createDeliveryBatch']) {
  assert.ok(validationUi.includes(`id="${id}"`), `No aparece el botón ${id}.`);
...
console.log('OK: todos los controles del módulo de Validación tienen enlace de evento en V9.2.14.');
