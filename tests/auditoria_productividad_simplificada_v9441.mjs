...
const checks=[
  ...
  ['diseño aprovecha el ancho administrativo',/body \.main:has\(\.productivity-page\)\{max-width:none;margin:0\}/.test(css)&&/grid-template-columns:repeat\(4,minmax\(0,1fr\)\)/.test(css)],
  ['alertas operativas se distinguen de incidencias de vinculación',/function productivityAlertBadgeV9441/.test(main)&&/Alertas operativas/.test(render)&&/distintas de los registros pendientes de vinculación/.test(render)],
  ['recálculo del módulo no se confunde con actualización global',/Recalcular indicadores/.test(render)],
  ['empleado sin actividad usa redacción singular correcta',/1 empleado activo sin actividad en el período/.test(render)&&/empleados activos sin actividad en el período/.test(render)],
