...
$backupDir = Join-Path $projectDir "backups\V9.3.7.5_$timestamp"
$excludedNames = @('APLICAR_V9375.ps1', 'LEEME_V9.3.7.5.txt')

Write-Host ''
Write-Host 'Productos Cesar CRM - V9.3.7.5 AUDITADA' -ForegroundColor Cyan
Write-Host 'Areas operativas multiples por empleado y estaciones compartidas'
Write-Host ''
...
Write-Host ''
Write-Host 'V9.3.7.5 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Ejecuta primero el SQL 34 en Supabase y luego npm.cmd run dev para la prueba local.' -ForegroundColor Cyan
