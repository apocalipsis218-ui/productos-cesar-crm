-- V9.4.4 - Productividad integral mensual por empleado y rol.
--
-- Corrige el cálculo histórico que se ejecutaba sobre el subconjunto cargado
-- en el navegador y agrupaba únicamente por nombre. La RPC usa el mes completo,
-- separa empleado + rol, deduplica clientes y toma los cobros por su fecha real.

create index if not exists idx_ordenes_productividad_preparado_v944
  on public.ordenes (preparado_en, tomado_por_empleado_id)
  where preparado_en is not null and estado not in ('Anulado', 'Cancelado');

create index if not exists idx_ordenes_productividad_facturado_v944
  on public.ordenes (facturado_en)
  where facturado_en is not null and estado not in ('Anulado', 'Cancelado');

create index if not exists idx_ordenes_productividad_validado_v944
  on public.ordenes (validado_en)
  where validado_en is not null and estado not in ('Anulado', 'Cancelado');

create index if not exists idx_orden_pagos_productividad_v944
  on public.orden_pagos (creado_en, orden_id)
  where coalesce(reversado, false) = false;

create index if not exists idx_entrega_lotes_productividad_v944
  on public.entrega_lotes ((coalesce(fecha_entrega, creado_en)), responsable_empleado_id)
  where coalesce(estado, '') <> 'Revertido';

create index if not exists idx_liquidaciones_productividad_v944
  on public.liquidaciones_lotes (fecha_liquidacion)
  where coalesce(estado, '') <> 'Revertido';

create index if not exists idx_llamadas_productividad_v944
  on public.llamadas (fecha, vendedor);
...
begin
  ...
  prep_role as (
    select
      ...
      count(*) filter (where p.validada)::bigint as operaciones_validas,
      0::bigint as viajes,
  ...
  configured_rows as materialized (
    select
      ...
      case
        ...
        when r.rol = 'Delivery' then r.clientes_validos::numeric
        when r.rol = 'Despachador' and coalesce(v_cfg #>> '{despachador,base}', 'cliente_despachado') = 'orden'
          then case
            when coalesce((v_cfg #>> '{despachador,cuentaSoloValidadas}')::boolean, true)
              then r.operaciones_validas::numeric
            else r.operaciones::numeric
          end
        when r.rol = 'Despachador' then case
          when coalesce((v_cfg #>> '{despachador,cuentaSoloValidadas}')::boolean, true)
            then r.clientes_validos::numeric
          else r.clientes_unicos::numeric
        end
...
comment on function public.resumen_productividad_mensual_v944(date) is
  'Productividad mensual integral por empleado y rol. Usa el mes completo, clientes únicos, cobros no reversados por fecha real y autorización del módulo Productividad.';
