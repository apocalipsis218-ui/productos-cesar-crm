-- V9.4.3 R1 - Progreso mensual confiable para Carnicería.
-- La métrica usa la fecha real de finalización en Santo Domingo y se calcula
-- en el servidor para no depender del subconjunto incremental del navegador.

create index if not exists idx_ordenes_carniceria_progreso_v943
  on public.ordenes (tomado_por_empleado_id, preparado_en)
  where preparado_en is not null and estado <> 'Anulado';
...
begin
  ...
  select jsonb_build_object(
    ...
    'tiempo_promedio_minutos', coalesce(round(avg(
      case
        when tomado_en is not null and preparado_en >= tomado_en
          then extract(epoch from (preparado_en - tomado_en)) / 60
        else null
      end
    ), 1), 0),
...
comment on function public.resumen_carniceria_mensual_v943(bigint, date) is
  'Resumen mensual de Carnicería por empleado. Distingue clientes registrados, ocasionales por teléfono y pedidos sin identidad reutilizable.';
