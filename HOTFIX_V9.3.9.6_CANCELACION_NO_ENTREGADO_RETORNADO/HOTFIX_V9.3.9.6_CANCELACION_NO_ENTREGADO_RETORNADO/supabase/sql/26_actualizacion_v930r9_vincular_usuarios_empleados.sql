-- =========================================================
-- PRODUCTOS CÉSAR CRM · V9.3.0 R9 · SQL 26 CORREGIDO
...
--
-- Corrección 20/07/2026:
--   Crea perfiles.correo antes de sincronizarlo desde auth.users.
--   El script anterior fallaba con: column p.correo does not exist.
--
...
-- ---------------------------------------------------------
-- 5. HISTORIAL DE CAMBIOS (AUTOCONTENIDO)
-- Se asegura porque algunas instalaciones antiguas no ejecutaron
-- completamente el SQL 24 de Usuarios y permisos.
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 6. AUTORIZACIÓN PARA ADMINISTRAR USUARIOS
...
drop policy if exists v930r9_historial_usuarios_select on public.usuarios_permisos_historial;
create policy v930r9_historial_usuarios_select
on public.usuarios_permisos_historial
...
select
  exists(select 1 from information_schema.columns where table_schema='public' and table_name='perfiles' and column_name='correo') as correo_creado,
  ...
  to_regclass('public.usuarios_permisos_historial') is not null as historial_disponible,
