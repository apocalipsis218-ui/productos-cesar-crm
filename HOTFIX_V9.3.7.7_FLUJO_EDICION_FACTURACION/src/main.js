...
// V9.3.7.7 · Módulo de Órdenes compacto, paginado y priorizado para operación diaria.
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7.7 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7.7 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7.7 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7.7 PWA</span></div>
...
function openOrderForm(o=null, client=null, extra={}){
  ...
  $('#saveOrder',m).onclick=async()=>{
    ...
    if(o&&composition.changed){
      const ok=confirm('Cambiaste la composición de una orden que ya estaba en proceso. El pesaje anterior será invalidado y la orden volverá a Carnicería para confirmar nuevamente los productos y el peso.\n\n¿Deseas continuar?');
      if(!ok) return;
