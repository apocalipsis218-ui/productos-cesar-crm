...
$backupDir = Join-Path $projectDir "backups\V9.3.9.5_$timestamp"
$excludedNames = @('LEEME_V9.3.9.5.txt')

Write-Host 'Productos Cesar CRM - V9.3.9.5 AUDITADA' -ForegroundColor Cyan
Write-Host 'Correccion del retorno de pedidos no entregados a Validacion'
...
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\48_actualizacion_v9395_corregir_no_entregado_validacion.sql'))) {
  throw 'Paquete incompleto: falta el SQL 48 correctivo.'
}
...
Write-Host ''
Write-Host 'V9.3.9.5 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Ejecuta el SQL 48 en Supabase antes de repetir la liquidacion.' -ForegroundColor Yellow
