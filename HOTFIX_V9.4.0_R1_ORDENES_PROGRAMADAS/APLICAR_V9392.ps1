...
$backupDir = Join-Path $projectDir "backups\V9.3.9.2_$timestamp"
$excludedNames = @('LEEME_V9.3.9.2.txt')

Write-Host 'Productos Cesar CRM - V9.3.9.2 AUDITADA' -ForegroundColor Cyan
Write-Host 'Devolucion parcial por articulos, monto y peso netos'
...
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\45_actualizacion_v9392_devolucion_parcial_articulos.sql'))) {
  throw 'Paquete incompleto: falta el SQL 45 obligatorio.'
}
...
Write-Host ''
Write-Host 'V9.3.9.2 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'IMPORTANTE: ejecuta en Supabase el SQL 45 despues del SQL 44:'
Write-Host 'supabase\sql\45_actualizacion_v9392_devolucion_parcial_articulos.sql' -ForegroundColor Yellow
