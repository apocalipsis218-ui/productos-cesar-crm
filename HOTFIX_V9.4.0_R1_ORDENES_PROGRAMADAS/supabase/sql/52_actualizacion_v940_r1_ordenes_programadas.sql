-- =========================================================
-- 52 - V9.4.0 R1 · TOMA SEGURA DE ÓRDENES PROGRAMADAS
-- Productos César CRM
--
-- Corrige el flujo:
--   Programada (fecha alcanzada) -> En preparación
--
-- Garantías:
--   1) La fecha se valida en el servidor con horario dominicano.
--   2) Las órdenes futuras continúan bloqueadas.
--   3) La toma conserva empleado, cuenta, límite de cola y auditoría.
...
-- ---------------------------------------------------------
-- 1. PROGRAMACIÓN: FECHA DOMINICANA Y BLOQUEO EXPLÍCITO
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 2. IDENTIDAD: PROGRAMADA VENCIDA TAMBIÉN ES UNA TOMA
-- ---------------------------------------------------------
    ...
    if v_empleado.id is not null then
      if lower(unaccent(coalesce(v_empleado.area,'')))<>'carniceria'
         and not exists(
           select 1
           from unnest(
             coalesce(v_empleado.areas_adicionales,'{}'::text[])
           ) a
...
-- ---------------------------------------------------------
-- 3. TOMA CONCURRENTE Y TRANSACCIONAL
-- ---------------------------------------------------------
...
begin
  ...
  from public.cambiar_estado_orden_v9382(
    ...
    coalesce(
      ...
      'Orden tomada mediante control de programación V9.4.0 R1'
    ),
    'carniceria'
  );
end;
...
comment on function public.fn_orden_programacion_flags is
  'V9.4.0 R1: usa la fecha dominicana y bloquea la activación anticipada.';
comment on function public.pc_validar_identidad_preparacion_v9397 is
  'V9.4.0 R1: protege identidad al tomar Pedido recibido o Programada vencida.';
comment on function public.tomar_orden_v9397 is
  'V9.4.0 R1: permite tomar Programada solo cuando llegó su fecha de despacho.';
...
select
  ...
  )>0 as identidad_programada_protegida,
  exists(
    select 1
    from pg_trigger
