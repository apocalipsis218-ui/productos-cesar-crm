...
$backupDir = Join-Path $projectDir "backups\V9.4.0_R1_$timestamp"
$excludedNames = @('LEEME_V9.4.0_R1.txt')

Write-Host 'Productos Cesar CRM - V9.4.0 R1 AUDITADA' -ForegroundColor Cyan
Write-Host 'Toma segura de ordenes programadas al llegar su fecha'
...
$requiredSql = 45..52 | ForEach-Object {
  Get-ChildItem -Path (Join-Path $hotfixDir 'supabase\sql') `
    -Filter "$($_)_*.sql" `
    -File
}
if ($requiredSql.Count -ne 8) {
  throw 'Paquete incompleto: deben existir exactamente los SQL 45, 46, 47, 48, 49, 50, 51 y 52.'
}
...
Write-Host ''
Write-Host 'V9.4.0 R1 aplicada, auditada y compilada correctamente.' `
  -ForegroundColor Green
Write-Host 'Ahora ejecuta el SQL 52 en Supabase. No publiques antes de aplicarlo.' `
