...
Write-Host ""
Write-Host "Productos Cesar CRM - V9.3.0 R9" -ForegroundColor Cyan
Write-Host "Vinculacion de Usuarios con Empleados" -ForegroundColor Cyan
Write-Host ""
...
if (($current -notmatch 'V9\.3\.0 R8') -and ($current -notmatch 'V9\.3\.0 R9')) {
  throw "La base no parece ser V9.3.0 R8/R9. No se sobrescribio ningun archivo."
}

$stamp = Get-Date -Format 'yyyy-MM-dd_HH-mm-ss'
$backup = Join-Path $projectRoot ("respaldos\V9.3.0_R9_" + $stamp)
...
if (Test-Path (Join-Path $projectRoot 'package-lock.json')) { Copy-Item (Join-Path $projectRoot 'package-lock.json') (Join-Path $backup 'package-lock.json') -Force }
foreach($f in @('auditoria_usuarios_v9214.mjs','auditoria_despacho_compacto_v930r8.mjs','auditoria_usuarios_empleados_v930r9.mjs')){
...
Copy-Item (Join-Path $PSScriptRoot 'payload\package-lock.json') (Join-Path $projectRoot 'package-lock.json') -Force
Copy-Item (Join-Path $PSScriptRoot 'payload\tests\auditoria_usuarios_v9214.mjs') (Join-Path $projectRoot 'tests\auditoria_usuarios_v9214.mjs') -Force
...
Copy-Item (Join-Path $PSScriptRoot 'MAPEO_USUARIOS_EMPLEADOS_V930_R9.md') (Join-Path $projectRoot 'MAPEO_USUARIOS_EMPLEADOS_V930_R9.md') -Force
Copy-Item (Join-Path $PSScriptRoot 'APLICAR_V9.3.0_R9.md') (Join-Path $projectRoot 'APLICAR_V9.3.0_R9.md') -Force

Write-Host "OK: V9.3.0 R9 aplicada." -ForegroundColor Green
Write-Host "Respaldo: $backup" -ForegroundColor Yellow
Write-Host ""
Write-Host "IMPORTANTE: confirme que el SQL 26 fue ejecutado en Supabase." -ForegroundColor Yellow
Write-Host ""
