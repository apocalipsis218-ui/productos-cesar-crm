...
FOR INSERT
TO authenticated
WITH CHECK (public.mi_rol() IN ('Gerente','Administrador','Supervisor'));

CREATE POLICY sistema_configuracion_update
ON public.sistema_configuracion
FOR UPDATE
TO authenticated
USING (public.mi_rol() IN ('Gerente','Administrador','Supervisor'))
WITH CHECK (public.mi_rol() IN ('Gerente','Administrador','Supervisor'));
...
FOR DELETE
TO authenticated
USING (public.mi_rol() IN ('Gerente','Administrador'));
