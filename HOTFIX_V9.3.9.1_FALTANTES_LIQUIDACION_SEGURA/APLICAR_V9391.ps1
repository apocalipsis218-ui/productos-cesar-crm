...
$backupDir = Join-Path $projectDir "backups\V9.3.9.1_$timestamp"
$excludedNames = @('LEEME_V9.3.9.1.txt')

Write-Host 'Productos Cesar CRM - V9.3.9.1 AUDITADA' -ForegroundColor Cyan
Write-Host 'Liquidacion ocasional segura y seguimiento de faltantes'

if (-not (Test-Path (Join-Path $projectDir 'package.json'))) {
  throw 'No encontre package.json en la carpeta superior. Extrae el ZIP dentro de C:\Proyectos\productos-cesar-crm.'
}
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\44_actualizacion_v9391_faltantes_liquidacion_ocasional.sql'))) {
  throw 'Paquete incompleto: falta el SQL 44 obligatorio.'
}
...
Write-Host ''
Write-Host 'V9.3.9.1 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'IMPORTANTE: ejecuta en Supabase el SQL 44 despues del SQL 43:'
Write-Host 'supabase\sql\44_actualizacion_v9391_faltantes_liquidacion_ocasional.sql' -ForegroundColor Yellow
