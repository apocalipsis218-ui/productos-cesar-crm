...
// V9.3.7.6 · Módulo de Órdenes compacto, paginado y priorizado para operación diaria.
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7.6 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7.6 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7.6 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7.6 PWA</span></div>
...
function openOrderForm(o=null, client=null, extra={}){
  ...
  $('#saveOrder',m).onclick=async()=>{
    ...
    if(o&&composition.changed&&!composition.removalOnly){
      const ok=confirm('Cambiaste productos o cantidades de una orden que ya estaba en proceso. El pesaje anterior será invalidado y la orden volverá a Carnicería para prepararse y pesarse nuevamente.\n\n¿Deseas continuar?');
      if(!ok) return;
    ...
    if(o&&composition.changed){
      ...
      if(composition.removalOnly){
        const adjusted=Number(preparationPatch.peso_preparado||0);
        if(adjusted>0){
          await sb.from('orden_pesos').insert({orden_id:o.id,tipo:'Preparado',libras:adjusted,paquetes:o.paquetes_preparados||null,notas:`Peso recalculado por eliminación de producto(s): ${removedNames}. Referencia anterior invalidada.`,creado_por:state.user.id});
        }
        await logOrderState(o,o.estado,o.estado,`Productos eliminados en Facturación: ${removedNames}. Peso preparado recalculado a ${adjusted||0} lb; pesaje anterior invalidado.`);
      }else{
        await logOrderState(o,o.estado,'Pedido recibido',`Composición modificada después del pesaje. Se invalidaron pesos de preparación, facturación y validación; requiere nueva preparación en Carnicería.`);
      }
    }
