...
$backupDir = Join-Path $projectDir "backups\V9.3.7.4_$timestamp"
$excludedNames = @('APLICAR_V9374.ps1', 'LEEME_V9.3.7.4.txt')

Write-Host ''
Write-Host 'Productos Cesar CRM - V9.3.7.4 AUDITADA' -ForegroundColor Cyan
Write-Host 'Clientes ocasionales, estaciones, direcciones y ordenes recientes'
Write-Host ''
...
Write-Host ''
Write-Host 'V9.3.7.4 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Ejecuta primero el SQL 33 en Supabase y luego npm.cmd run dev para la prueba local.' -ForegroundColor Cyan
