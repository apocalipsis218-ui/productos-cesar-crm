...
function renderKanban(c){
  ...
  Object.keys(grouped).forEach(k=>grouped[k].sort((a,b)=>{ const sa=currentModuleOfOrder(a), sb=currentModuleOfOrder(b); const ma=sa?stageDurationFor(a,sa):minutesSince(createdAtOf(a)); const mb=sb?stageDurationFor(b,sb):minutesSince(createdAtOf(b)); return (mb||0)-(ma||0); }));
  c.innerHTML=`<div class="executive-hero kanban-hero"><div><div class="hero-eyebrow">V8.6 · Tablero Kanban</div><h3>Flujo completo de órdenes</h3><p>Visualiza dónde está cada pedido, quién lo tiene y cuánto tiempo lleva en su etapa actual.</p></div><div class="hero-actions"><button class="btn" data-go="alertas">Centro de alertas</button><button class="btn gray" data-go="ordenes">Lista de órdenes</button></div></div><div class="panel"><div class="panel-head"><div><h3>Tablero operativo</h3><p>${orders.length} orden(es) visibles. Arrastra visualmente el seguimiento por etapa; los cambios reales se hacen con los botones de cada módulo.</p></div></div><div class="searchbar"><input id="kanbanSearch" value="${esc(q)}" placeholder="Buscar orden, cliente, producto, teléfono o estado..."></div><div class="kanban-board">${cols.map(([id,title,sub])=>`<section class="kanban-col"><header><div><b>${esc(title)}</b><span>${esc(sub)}</span></div><em>${grouped[id].length}</em></header><div class="kanban-list">${grouped[id].slice(0,40).map(kanbanCard).join('')||'<div class="kanban-empty">Sin órdenes</div>'}</div></section>`).join('')}</div></div>`;
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.2.11</span></div>
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.2.11</span></div>
