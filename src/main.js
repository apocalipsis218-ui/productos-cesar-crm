...
function emptyValidationBatchDraft(){ return {date:today(),operationalDate:today(),deliveryValue:'',deliveryName:'',manual:'',responsibleType:RESPONSIBLE_TYPES.DELIVERY,rows:{}}; }
...
function saveBatchDeliveryDraft(container){
  ...
  const sel=$('#batchDelivery',container), man=$('#batchDeliveryManual',container), operationalDate=$('#batchOperationalDate',container);
  ...
  d.operationalDate=normalizeOperationalDate(operationalDate?.value)||today();
...
function productivityRoleActivityV9441(r){
  if(r.rol==='Despachador') return `${r.clientes} cliente(s) · ${r.ordenes} preparación(es) · ${Number(r.libras||0).toLocaleString('es-DO',{maximumFractionDigits:2})} lb`;
  if(r.rol==='Delivery') return `${r.clientes} cliente(s) · ${r.operacionesValidas} entrega(s) válida(s) · ${r.lotes} viaje(s)`;
  if(r.rol==='Vendedor') return `${r.clientes} cliente(s) · ${r.ordenes} venta(s) · ${money(r.montoCobrado)} cobrado`;
  if(r.rol==='Facturación') return `${r.ordenes} factura(s) · ${money(r.montoFacturado)} facturado`;
  if(r.rol==='Validación') return `${r.ordenes} validación(es) · ${r.operacionesValidas} válida(s)`;
  if(r.rol==='Liquidación') return `${r.ordenes} liquidación(es) · ${money(r.montoCobrado)} recibido`;
  if(r.rol==='Control') return `${r.clientes} cliente(s) · ${r.ordenes} gestión(es) · ${r.operacionesValidas} contactada(s)`;
  return `${r.clientes} cliente(s) · ${r.ordenes} actividad(es)`;
}
function productivityEmployeeSummariesV9441(rows){
  const grouped=new Map();
  rows.forEach(r=>{
    const key=String(r.empleadoId||r.empleado);
    if(!grouped.has(key)) grouped.set(key,{empleadoId:r.empleadoId,empleado:r.empleado,empleadoActivo:r.empleadoActivo,areaEmpleado:r.areaEmpleado,rows:[],incentivo:0,unidades:0,alertas:0});
    const item=grouped.get(key);
    item.rows.push(r);
    item.incentivo+=Number(r.incentivo||0);
    item.alertas+=r.alertas.length;
    if(r.tipoIncentivo!=='Sin incentivo') item.unidades+=Number(r.unidadesIncentivo||0);
  });
  return [...grouped.values()].map(item=>{
    item.rows.sort((a,b)=>(b.incentivo||0)-(a.incentivo||0)||(b.ordenes||0)-(a.ordenes||0)||a.rol.localeCompare(b.rol,'es'));
    item.primary=item.rows[0];
    item.roles=item.rows.map(r=>r.rol);
    return item;
  }).sort((a,b)=>b.incentivo-a.incentivo||b.unidades-a.unidades||a.empleado.localeCompare(b.empleado,'es'));
}
function productivityRoleBadgesV9441(item){
  const visible=item.roles.slice(0,3).map(role=>`<span class="badge info">${esc(role)}</span>`).join('');
  const extra=item.roles.length>3?`<span class="badge">+${item.roles.length-3}</span>`:'';
  return `<div class="productivity-role-badges">${visible}${extra}</div>`;
}
function productivityAlertBadgeV9441(count){
  const total=Number(count||0);
  if(!total) return '<span class="badge ok">OK</span>';
  const label=total===1?'alerta':'alertas';
  return `<span class="badge warn" title="Alertas internas reportadas por las funciones del empleado; son distintas de los registros pendientes de vinculación.">${total} ${label}</span>`;
}
function productivityRoleDetailCardsV9441(rows,baseHint){
  return `<div class="productivity-role-detail-grid">${rows.map(r=>`<article class="productivity-role-detail"><div><span class="badge info">${esc(r.rol)}</span><b>${esc(r.baseTexto)}</b><small>${esc(productivityRoleActivityV9441(r))}</small></div><div class="productivity-role-result"><span>${r.unidadesIncentivo} unidad(es) · ${baseHint(r)}</span><strong>${money(r.incentivo)}</strong></div></article>`).join('')}</div>`;
}
function productivityQualityItemsV9441(quality){
  const labels={
    preparaciones_sin_empleado:'Preparaciones sin despachador',
    viajes_sin_empleado:'Viajes sin delivery',
    ventas_cobradas_sin_empleado:'Cobros sin vendedor',
    ventas_facturadas_sin_empleado:'Ventas facturadas sin vendedor',
    facturas_sin_empleado:'Facturas sin responsable',
    validaciones_sin_empleado:'Validaciones sin responsable',
    liquidaciones_sin_empleado:'Liquidaciones sin responsable',
    llamadas_sin_empleado:'Gestiones sin responsable',
    empleados_con_nombre_duplicado:'Nombres de empleado duplicados'
  };
  return Object.entries(labels).map(([key,label])=>({key,label,value:Number(quality[key]||0)}));
}
function productivitySummaryViewV9441(employees,baseHint){
  const desktopRows=employees.map(item=>{
    const expanded=String(state.prodExpandedEmployeeId)===String(item.empleadoId);
    return `<tr><td><b>${esc(item.empleado)}</b><div class="hint">${esc(item.areaEmpleado||'Empleado operativo')}${item.empleadoActivo?'':' · Inactivo'}</div></td><td>${productivityRoleBadgesV9441(item)}</td><td><b>${esc(item.primary.rol)}</b><div class="hint">${esc(productivityRoleActivityV9441(item.primary))}</div></td><td><b>${item.unidades}</b><div class="hint">Solo funciones con regla</div></td><td><b>${money(item.incentivo)}</b></td><td>${productivityAlertBadgeV9441(item.alertas)}</td><td><button class="btn small gray" data-prod-employee="${item.empleadoId}">${expanded?'Ocultar':'Ver detalle'}</button></td></tr>${expanded?`<tr class="productivity-expanded-row"><td colspan="7">${productivityRoleDetailCardsV9441(item.rows,baseHint)}</td></tr>`:''}`;
  }).join('');
  const mobileCards=employees.map(item=>`<article class="productivity-mobile-card employee-summary"><div class="productivity-mobile-head"><div><h4>${esc(item.empleado)}</h4><p>${esc(item.areaEmpleado||'Empleado operativo')}${item.empleadoActivo?'':' · Inactivo'}</p></div><b>${money(item.incentivo)}</b></div>${productivityRoleBadgesV9441(item)}<div class="productivity-mobile-metrics"><div><span>Actividad principal</span><b>${esc(item.primary.rol)}</b><small>${esc(productivityRoleActivityV9441(item.primary))}</small></div><div class="primary"><span>Unidades incentivo</span><b>${item.unidades}</b><small>${item.alertas?`${item.alertas} ${item.alertas===1?'alerta operativa':'alertas operativas'}`:'Sin alertas operativas'}</small></div></div><button class="btn small gray productivity-mobile-detail-btn" data-prod-employee="${item.empleadoId}">${String(state.prodExpandedEmployeeId)===String(item.empleadoId)?'Ocultar detalle':'Ver detalle'}</button>${String(state.prodExpandedEmployeeId)===String(item.empleadoId)?productivityRoleDetailCardsV9441(item.rows,baseHint):''}</article>`).join('');
  return `<div class="panel panel-clean productivity-ranking-panel"><div class="panel-head"><div><h3>Resumen por empleado</h3><p>Una sola fila por persona. El desglose de sus funciones se abre cuando lo necesitas.</p></div><span class="badge info">${employees.length} empleado(s)</span></div><div class="productivity-mobile-list">${mobileCards||'<div class="empty">No hay actividad registrada para este filtro.</div>'}</div><div class="table-wrap productivity-table-wrap"><table class="table productivity-table productivity-summary-table"><thead><tr><th>Empleado</th><th>Funciones</th><th>Actividad principal</th><th>Unidades incentivo</th><th>Incentivo</th><th>Alertas operativas</th><th></th></tr></thead><tbody>${desktopRows||'<tr><td colspan="7">No hay actividad registrada para este filtro.</td></tr>'}</tbody></table></div></div>`;
}
function productivityRoleViewV9441(rows,baseHint){
  return `<div class="panel panel-clean productivity-ranking-panel"><div class="panel-head"><div><h3>Detalle por función</h3><p>Vista de auditoría: cada fila representa una función concreta del empleado.</p></div><span class="badge info">${rows.length} función(es)</span></div><div class="productivity-mobile-list">${rows.map(r=>`<article class="productivity-mobile-card"><div class="productivity-mobile-head"><div><h4>${esc(r.empleado)}</h4><p>${esc(r.areaEmpleado||'Empleado operativo')}</p></div><span class="badge info">${esc(r.rol)}</span></div><div class="productivity-mobile-metrics"><div><span>Actividad</span><b>${r.ordenes}</b><small>${esc(productivityRoleActivityV9441(r))}</small></div><div class="primary"><span>Incentivo</span><b>${money(r.incentivo)}</b><small>${r.unidadesIncentivo} unidad(es) · ${baseHint(r)}</small></div></div></article>`).join('')||'<div class="empty">No hay actividad registrada para este filtro.</div>'}</div><div class="table-wrap productivity-table-wrap"><table class="table productivity-table productivity-role-table"><thead><tr><th>Empleado</th><th>Función</th><th>Actividad</th><th>Actividad válida</th><th>Regla aplicada</th><th>Incentivo</th><th>Alertas operativas</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${esc(r.empleado)}</b><div class="hint">${esc(r.areaEmpleado||'Empleado operativo')}</div></td><td><span class="badge info">${esc(r.rol)}</span></td><td><b>${r.ordenes}</b><div class="hint">${esc(productivityRoleActivityV9441(r))}</div></td><td>${r.operacionesValidas}<div class="hint">${r.clientesValidos} cliente(s) con actividad válida</div></td><td><b>${esc(r.baseTexto)}</b><div class="hint">${r.unidadesIncentivo} unidad(es) · ${baseHint(r)}</div></td><td><b>${money(r.incentivo)}</b></td><td>${productivityAlertBadgeV9441(r.alertas.length)}</td></tr>`).join('')||'<tr><td colspan="7">No hay productividad registrada para este período.</td></tr>'}</tbody></table></div></div>`;
}
function productivityIssuesViewV9441(quality,noActivity){
  const issues=productivityQualityItemsV9441(quality).filter(item=>item.value>0);
  const reversed=Number(quality.pagos_reversados||0);
  const noActivityLabel=noActivity.length===1?'1 empleado activo sin actividad en el período':`${noActivity.length} empleados activos sin actividad en el período`;
  return `<div class="panel panel-clean productivity-issues-panel"><div class="panel-head"><div><h3>Incidencias de vinculación</h3><p>Estos registros permanecen fuera del resultado individual hasta que se asigne el empleado correcto.</p></div><span class="badge ${issues.length?'warn':'ok'}">${issues.reduce((s,item)=>s+item.value,0)} pendiente(s)</span></div>${issues.length?`<div class="productivity-issue-grid">${issues.map(item=>`<article class="productivity-issue-card"><strong>${item.value}</strong><span>${esc(item.label)}</span><small>Requiere corrección administrativa</small></article>`).join('')}</div>`:'<div class="success">No hay registros productivos pendientes de identificar.</div>'}${reversed?`<div class="productivity-info-note"><b>${reversed} pago(s) reversado(s)</b><span>Se conservaron para auditoría y no se incluyeron en el cobrado ni en incentivos.</span></div>`:''}${noActivity.length?`<details class="productivity-no-activity"><summary>${noActivityLabel}</summary><div>${noActivity.map(e=>`<span class="badge">${esc(e.empleado_nombre)}</span>`).join('')}</div></details>`:''}</div>`;
}
function renderProductividad(c){
  ...
  const view=['resumen','roles','incidencias'].includes(state.prodView)?state.prodView:'resumen';
  ...
  const employees=productivityEmployeeSummariesV9441(rows);
  const totalIncentivo=rows.reduce((s,r)=>s+Number(r.incentivo||0),0);
  const totalUnits=rows.filter(r=>r.tipoIncentivo!=='Sin incentivo').reduce((s,r)=>s+Number(r.unidadesIncentivo||0),0);
  const summary=state.productivitySummary?.resumen||{};
  const quality=state.productivitySummary?.calidad||{};
  const qualityItems=productivityQualityItemsV9441(quality);
  const missingIdentity=qualityItems.reduce((s,item)=>s+item.value,0);
  const totalClientes=role==='Todos'?Number(summary.clientes_unicos_equipo||0):rows.reduce((s,r)=>s+Number(r.clientes||0),0);
  const employeeCount=role==='Todos'?Number(summary.empleados_con_actividad||0):employees.length;
  ...
  const qualityPanel=state.productivitySchemaOk&&missingIdentity>0
    ? `<div class="productivity-quality-banner"><div><span class="badge warn">Revisión</span><b>${missingIdentity} registro(s) pendiente(s) de asignar</b><small>No se atribuyen a ningún empleado hasta corregir su identidad.</small></div><button class="btn small gray" data-prod-view="incidencias">Revisar</button></div>`
    : '';
  ...
  const content=view==='roles'
    ? productivityRoleViewV9441(rows,baseHint)
    : view==='incidencias'
      ? productivityIssuesViewV9441(quality,noActivity)
      : productivitySummaryViewV9441(employees,baseHint);
  c.innerHTML=`<div class="productivity-page"><div class="executive-hero productivity-hero"><div><div class="hero-eyebrow">V9.4.4.1 · Productividad simplificada</div><h3>Productividad mensual del equipo</h3><p>Resumen por empleado para revisar actividad e incentivos sin mezclar la auditoría técnica.</p></div><div class="hero-actions"><button class="btn" data-go="config" data-config-go="incentivos">Configurar</button><button class="btn gray" id="refreshProd">Recalcular indicadores</button><button class="btn dark" id="printProd">Imprimir</button></div></div>
  ...
  <div class="exec-kpi-grid productivity-kpis"><div class="exec-kpi primary"><span>Incentivo estimado</span><strong>${money(totalIncentivo)}</strong><small>Según las reglas activas</small></div><div class="exec-kpi"><span>Clientes únicos</span><strong>${totalClientes}</strong><small>${role==='Todos'?'Sin duplicar en el equipo':'Únicos por empleado en esta función'}</small></div><div class="exec-kpi"><span>Unidades incentivables</span><strong>${totalUnits}</strong><small>Pedidos, entregas o ventas según la regla</small></div><div class="exec-kpi"><span>Empleados productivos</span><strong>${employeeCount}</strong><small>Con actividad registrada en el mes</small></div></div>
  <div class="productivity-view-tabs tabs"><button class="tab ${view==='resumen'?'active':''}" data-prod-view="resumen">Resumen por empleado</button><button class="tab ${view==='roles'?'active':''}" data-prod-view="roles">Detalle por rol</button><button class="tab ${view==='incidencias'?'active':''}" data-prod-view="incidencias">Incidencias${missingIdentity?` (${missingIdentity})`:''}</button></div>
  ${content}</div>`;
  ...
  $('#prodMonth').onchange=async e=>{state.prodMonth=e.target.value;state.prodExpandedEmployeeId=null;await reload();};
  $('#prodYear').onchange=async e=>{state.prodYear=e.target.value;state.prodExpandedEmployeeId=null;await reload();};
  $('#prodRole').onchange=e=>{state.prodRole=e.target.value;state.prodExpandedEmployeeId=null;renderProductividad($('#content'));};
  $$('[data-prod-view]',c).forEach(b=>b.onclick=()=>{state.prodView=b.dataset.prodView;state.prodExpandedEmployeeId=null;renderProductividad($('#content'));});
  $$('[data-prod-employee]',c).forEach(b=>b.onclick=()=>{state.prodExpandedEmployeeId=String(state.prodExpandedEmployeeId)===String(b.dataset.prodEmployee)?null:b.dataset.prodEmployee;renderProductividad($('#content'));});
  $('#refreshProd').onclick=async()=>{await reload();toast('Productividad actualizada');};
  $('#printProd').onclick=()=>window.print();
  $$('[data-go="config"]',c).forEach(b=>b.onclick=()=>{state.page='config';state.configTab=b.dataset.configGo||'incentivos';render();});
}
...
function normalizeOperationalDate(value){
  ...
  if(isNaN(dt.getTime()) || dt.toISOString().slice(0,10)!==raw) return '';
  return raw;
}
function validateOperationalDate(value){
  const normalized=normalizeOperationalDate(value);
  if(!normalized) return {ok:false,value:'',message:'Selecciona una fecha válida para el lote.'};
  if(normalized>today()) return {ok:false,value:normalized,message:'La fecha del lote no puede ser posterior a la fecha actual.'};
  return {ok:true,value:normalized,message:''};
}
function lotOperationalDate(lot){
  const snap=normalizeRouteSnapshot(lot?.hoja_ruta_snapshot);
  return normalizeOperationalDate(lot?.fecha_operativa)||normalizeOperationalDate(snap?.fecha_operativa)||businessDateKey(lot?.fecha_entrega||lot?.creado_en||'');
}
function lotRecordedAt(lot){
  const snap=normalizeRouteSnapshot(lot?.hoja_ruta_snapshot);
  return lot?.fecha_entrega||snap?.fecha_entrega||lot?.creado_en||'';
}
...
function buildDeliveryRouteSnapshot(lote,deliveryName,selected,originalDate=new Date().toISOString(),validatedBy=currentWorkerName(),operationalDate=businessDateKey(originalDate)){
  ...
  return {version:'V9.4.5.3 PWA',codigo_lote:lote,delivery_nombre:deliveryName,fecha_operativa:normalizeOperationalDate(operationalDate)||businessDateKey(originalDate),fecha_entrega:originalDate,validado_por:validatedBy,empresa:deliveryRouteSnapshotCompany(),items,totales:{cantidad_ordenes:items.length,total_facturado:items.reduce((a,x)=>a+x.monto_factura,0),peso_esperado:items.reduce((a,x)=>a+x.peso_esperado,0),peso_entregado:items.reduce((a,x)=>a+x.peso_entregado,0)}};
}
...
function validationHistoryFilteredRows(){
  ...
  return validationBatchRows().filter(l=>validationDateInRange(lotOperationalDate(l),from,to) && (!del||l.delivery_nombre===del) && (!q || norm(l.codigo_lote).includes(q) || norm(l.delivery_nombre).includes(q) || validationBatchRouteItems(l).some(x=>norm([x.o?.codigo,x.o?.factura_no,x.o?.cliente?.negocio,x.o?.cliente?.telefono].join(' ')).includes(q))));
}
...
async function saveFormalDeliveryBatch(lote, deliveryName, selected, meta={}){
  const originalDate=meta.originalDate||new Date().toISOString(); const operationalDate=normalizeOperationalDate(meta.operationalDate)||businessDateKey(originalDate); const validatedBy=meta.validatedBy||currentWorkerName(); const snapshot=meta.snapshot||buildDeliveryRouteSnapshot(lote,deliveryName,selected,originalDate,validatedBy,operationalDate);
  const basePayload={codigo_lote:lote,delivery_nombre:deliveryName,fecha_operativa:operationalDate,fecha_entrega:originalDate,cantidad_ordenes:selected.length,peso_esperado:selected.reduce((sum,x)=>sum+Number(x.expected||0),0),peso_entregado:selected.reduce((sum,x)=>sum+Number(x.peso||0),0),total_facturado:selected.reduce((sum,x)=>sum+Number(x.amount||0),0),estado:'Abierto',creado_por:state.user?.id||null};
  ...
  if(result.error){ const {fecha_operativa,...compatiblePayload}=basePayload; console.warn('Esquema V9.4.5.3 no disponible; guardando lote compatible:',result.error.message); result=await sb.from('entrega_lotes').insert(compatiblePayload).select('*').single(); }
...
function validationHistoryCard(l,index=0,forceOpen=false){
  const items=validationBatchRouteItems(l); const st=validationBatchCurrentState(l); const operationalDate=lotOperationalDate(l); const recordedAt=lotRecordedAt(l); const audits=validationBatchPrintAudit(l.codigo_lote); const reprints=Number(l.cantidad_reimpresiones||audits.filter(x=>x.tipo_evento==='Reimpresión').length||0);
  const key=lotUiKey('validation',l.codigo_lote,l.id||recordedAt); const open=operationalLotOpen('validationLots',key,index,forceOpen); const info=validationLotCorrectionInfo(l); const correction=lastLotCorrection(l);
...
function updateBatchSummary(container,orders){
  ...
  const operationalDate=normalizeOperationalDate($('#batchOperationalDate',container)?.value)||today();
  ...
  const box=$('#batchSummary',container); if(box){ box.className='lock-alert '+(operationalDate<today()?'warn':(sel.length?'ok':'')); box.innerHTML=sel.length?`<b>Lote en preparación:</b> Fecha ${operationalDateLabel(operationalDate)} · ${sel.length} orden(es) · Peso esperado ${Number(expected.toFixed(2))} lb · Peso entregado ${Number(peso.toFixed(2))} lb · Monto final ${money(total)}.`:`Fecha del lote: <b>${operationalDateLabel(operationalDate)}</b>. Selecciona las órdenes que llevará el responsable.`; }
}
...
function bindValidationBatch(container,orders){
  const delivery=$('#batchDelivery',container), manual=$('#batchDeliveryManual',container), operationalDate=$('#batchOperationalDate',container);
  ...
  $('#previewBatchRoute',container).onclick=()=>{ const val=validateSelectedBatch(container,orders); const del=getBatchDelivery(container)||'—'; const dateCheck=validateOperationalDate(operationalDate?.value); if(!dateCheck.ok) return alert(dateCheck.message); if(!val.selected.length) return alert('Selecciona al menos una orden.'); if(val.missingAmounts.length) return alert('Falta el monto final de factura en estas órdenes:\n- '+val.missingAmounts.join('\n- ')); printDeliveryBatchSheet(del,newBatchCode(),val.selected,false,{operationalDate:dateCheck.value,recordedAt:new Date().toISOString(),validatedBy:currentWorkerName()}); };
  $('#createDeliveryBatch',container).onclick=async()=>{
    ...
    const fechaOriginal=new Date().toISOString(); const fechaOperativa=dateCheck.value; const validadoPor=currentWorkerName();
    const snapshot=buildDeliveryRouteSnapshot(lote,responsible.name,val.selected,fechaOriginal,validadoPor,fechaOperativa);
    ...
    const {data,error}=await sb.rpc('crear_lote_entrega_v9453',{
      p_codigo_lote:lote,p_responsable_nombre:responsible.name,p_responsable_tipo:responsible.type,
      p_items:items,p_validado_por:validadoPor,p_fecha_operativa:fechaOperativa,p_snapshot:snapshot
    });
    ...
    printDeliveryBatchSheet(responsible.name,lote,val.selected,true,{operationalDate:fechaOperativa,recordedAt:fechaOriginal,validatedBy:validadoPor,reprint:false,responsibleType:responsible.type});
    await recordDeliveryDocumentEvent(lote,'Hoja de ruta','Original',{fecha_original:fechaOriginal,fecha_operativa:fechaOperativa,cantidad_ordenes:val.selected.length,responsable_tipo:responsible.type});
    clearValidationBatchDraft();
    await refreshVisibleModuleV9384(); render(); toast(`Lote ${data?.codigo_lote||lote} · ${operationalDateLabel(fechaOperativa)} · asignado a ${responsible.name}`);
  };
  updateBatchSummary(container,orders);
}
function printDeliveryBatchSheet(deliveryName,lote,items,auto=true,opts={}){
  const recordedAt=opts.recordedAt||opts.originalDate||new Date().toISOString(), operationalDate=normalizeOperationalDate(opts.operationalDate)||businessDateKey(recordedAt), printDate=new Date().toISOString(), isReprint=opts.reprint===true;
...
function printValidationRouteFromLot(lot,auto=true){
  ...
  printDeliveryBatchSheet(lot.delivery_nombre,lot.codigo_lote,items,auto,{operationalDate:lotOperationalDate(lot),recordedAt:lotRecordedAt(lot),validatedBy:snap?.validado_por||lot.validado_por||'—',reprint:true});
  recordDeliveryDocumentEvent(lot.codigo_lote,'Hoja de ruta','Reimpresión',{fecha_original:lotRecordedAt(lot)||null,fecha_operativa:lotOperationalDate(lot),cantidad_ordenes:items.length});
}
function printValidationDeliveryReceipt(lot,auto=true){
  ...
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>Constancia ${esc(lot.codigo_lote)}</title><style>body{font-family:Arial,sans-serif;padding:22px;color:#111;font-size:12px}table{width:100%;border-collapse:collapse;margin-top:14px}th,td{border:1px solid #ddd;padding:7px;text-align:left}th{background:#f3f4f6}.box{border:1px solid #ddd;border-radius:10px;padding:12px;margin:12px 0}.sign{display:inline-block;width:240px;border-top:1px solid #000;margin:40px 35px 0 0;padding-top:4px}@media print{button{display:none}}</style></head><body>${printCompanyHeader('Constancia de entrega a delivery','Historial de Validación')}<div class="box"><b>Lote:</b> ${esc(lot.codigo_lote)}<br><b>Fecha del lote:</b> ${operationalDateLabel(lotOperationalDate(lot))}<br><b>Registrado en el sistema:</b> ${businessDateTime(lotRecordedAt(lot))}<br><b>Responsable del viaje:</b> ${esc(tripResponsibleName(lot)||'—')}<br><b>Validado por:</b> ${esc(snap?.validado_por||lot.validado_por||'—')}<br><b>Estado actual:</b> ${esc(validationBatchCurrentState(lot))}</div><p><b>Órdenes:</b> ${items.length} · <b>Total:</b> ${money(total)} · <b>Peso entregado:</b> ${Number(peso.toFixed(2))} lb</p><table><thead><tr><th>Orden</th><th>Cliente</th><th>Factura</th><th>Monto</th><th>Peso</th><th>Estado actual</th></tr></thead><tbody>${rows}</tbody></table>${signatureHtml(appCfg('recibos.firmaValidacion','Entregado por validación'))}${signatureHtml(appCfg('recibos.firmaDelivery','Recibido por delivery'))}${printFooterHtml()}<button onclick="window.print()">Imprimir</button>${auto?'<script>setTimeout(()=>window.print(),500)<\/script>':''}</body></html>`;
  const w=window.open('','_blank','width=950,height=720'); if(!w) return alert('Permite ventanas emergentes para imprimir.'); w.document.write(html);w.document.close(); recordDeliveryDocumentEvent(lot.codigo_lote,'Constancia de entrega','Reimpresión',{fecha_original:lotRecordedAt(lot)||null,fecha_operativa:lotOperationalDate(lot),cantidad_ordenes:items.length});
}
...
function deliveryActiveGroupCard(g,index=0,forceOpen=false){
  ...
  const lot=g.displayCode==='SIN-LOTE'?null:batchRecordByCode(g.displayCode); const operationalDate=lot?lotOperationalDate(lot):'';
...
function liquidacionBatchGroups(orders){
  ...
  return [...map.entries()].map(([code,items])=>{ const lot=code==='SIN-LOTE'?null:batchRecordByCode(code); return {code,items,summary:deliveryMoneySummary(items),date:code==='SIN-LOTE'?'Órdenes individuales / anteriores':(lot?`Fecha del lote: ${operationalDateLabel(lotOperationalDate(lot))}`:batchDateFromCode(code))}; }).sort((a,b)=>{
...
async function receiveOrderCxcV945(o,payload){
  ...
  const {data,error}=await sb.rpc('recibir_orden_cxc_v945_r2',{
    p_orden_id:Number(o.id),
    p_resultado:payload.result,
    p_monto_recibido:Number(payload.cash||0),
    p_metodo:payload.method||'Efectivo',
    ...
    p_autorizar_sobrante:payload.authorizeSurplus===true,
    p_recibido_por:payload.receivedBy||currentWorkerName(),
    p_observacion:payload.note||null,
...
async function receiveBatchCxcV9392R2(g,payload){
  ...
  const {data,error}=await sb.rpc('recibir_lote_cxc_v945_r2',{
    p_lote_id:Number(lot.id),
...
function historyDeliveryDate(l,items=[]){
  ...
  if(lot) return lotOperationalDate(lot);
  if(normalizeOperationalDate(l?.fecha_operativa)) return normalizeOperationalDate(l.fecha_operativa);
...
function cashHistoryStatusBadgeV9452(row,count){
  const difference=Number(count?.diferencia??row?.diferencia??0);
  const surplus=Number(count?.sobrante_monto||0);
  if(count?.sobrante_autorizado===true && surplus>0.009){
    return `<span class="badge warn">Sobrante autorizado ${money(surplus)}</span>`;
  }
  if(difference>0.009) return `<span class="badge bad">Sobrante sin autorización ${money(difference)}</span>`;
  if(difference< -0.009) return `<span class="badge bad">Faltante ${money(Math.abs(difference))}</span>`;
  return `<span class="badge ok">Cuadrado</span>`;
}
function liquidacionHistoryCard(l,scope='liquidacion'){
  ...
  const deliveryLabel=normalizeOperationalDate(delivered)?operationalDateLabel(delivered):businessDateTime(delivered);
...
function openValidacionModal(o){
  ...
  if(valDelivery) valDelivery.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); focusEl(valOperationalDate); } });
  if(valOperationalDate) valOperationalDate.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); focusEl(valNotas); } });
  ...
  save.onclick=async()=>{
    ...
    const dateCheck=validateOperationalDate(valOperationalDate?.value);
    if(!dateCheck.ok) return alert(dateCheck.message);
    ...
    const {data,error}=await sb.rpc('crear_lote_entrega_v9453',{
      ...
      p_validado_por:by,p_fecha_operativa:fechaOperativa,p_snapshot:snapshot
    });
    ...
    m.remove(); await refreshVisibleModuleV9384(); render(); toast(`Orden asignada a ${del} · lote ${operationalDateLabel(fechaOperativa)} · ${data?.codigo_lote||lote}`);
  };
  focusEl(valMonto);
}
...
function paintCashBreakdownTriggerV945(root,id,reconciliation,options={}){
  ...
  if(options.badgeText){
    ...
    badge.textContent=`Sobrante ${money(reconciliation.difference)}`;
  }else if(reconciliation.hasShortage){
    badge.className='badge bad';
    badge.textContent=`Faltante ${money(Math.abs(reconciliation.difference))}`;
  }else{
    badge.className='badge warn';
...
function canAuthorizeCashSurplusV9452(){
  return ['Gerente','Administrador'].includes(String(state.profile?.rol||''));
}
function openCashBreakdownSheetV945({title='Desglose de efectivo',expected=0,rows=[],onApply}){
  ...
  const body=`<div class="form cash-sheet-form"><div class="cash-sheet-expected"><span>Efectivo esperado</span><strong>${money(expected)}</strong></div><div class="cash-sheet-list">${rowHtml}</div><div class="cash-sheet-totals"><div><span>Físico contado</span><b id="cashSheetCounted">${money(0)}</b></div><div><span>Ajuste</span><b id="cashSheetAdjustment">${money(0)}</b></div><div><span>Diferencia</span><b id="cashSheetDifference">${money(0)}</b></div></div><div id="cashSheetStatus" class="lock-alert warn">Digita la cantidad de cada denominación.</div><section id="cashSheetSurplusAuthorization" class="cash-surplus-authorization-v9452" hidden><div><b>Autorizar sobrante de efectivo</b><span>Esta autorización quedará registrada en el cierre.</span></div><dl><div><dt>Esperado</dt><dd id="cashSurplusExpected">${money(0)}</dd></div><div><dt>Físico</dt><dd id="cashSurplusCounted">${money(0)}</dd></div><div><dt>Sobrante</dt><dd id="cashSurplusAmount">${money(0)}</dd></div></dl><p id="cashSurplusActor"></p><div class="actions"><button type="button" class="btn gray" id="cancelCashSurplus">Cancelar</button><button type="button" class="btn warn" id="authorizeCashSurplus">Autorizar sobrante</button></div></section><div class="actions cash-sheet-actions"><button type="button" class="btn gray" id="clearCashSheet">Limpiar</button><button type="button" class="btn" id="applyCashSheet">Aplicar desglose</button></div></div>`;
  ...
  const inputs=$$('[data-cash-sheet-denomination]',sheet),apply=$('#applyCashSheet',sheet),authorization=$('#cashSheetSurplusAuthorization',sheet),authorize=$('#authorizeCashSurplus',sheet);
  let pendingAuthorizationKey='';
  function resetAuthorization(){
    pendingAuthorizationKey='';
    if(authorization) authorization.hidden=true;
  }
  function finish(value,authorized=false){
    onApply?.({...value,surplusAuthorized:authorized,surplusAuthorizationKey:authorized?cashBreakdownSurplusAuthorizationKeyV945(value):''});
  ...
  function paint(){
    ...
    try{
      ...
      value={breakdown:[],expected:Number(expected||0),counted:0,adjustment:0,difference:Number(expected||0)*-1,rawDifference:Number(expected||0)*-1,exact:false,hasSurplus:false,hasShortage:true,canApply:false,canClose:false,error:err.message||String(err)};
    }
    ...
    status.className='lock-alert '+(value.canClose?'ok':(value.hasSurplus?'warn':'bad'));
    status.textContent=value.canClose
      ...
      : value.hasSurplus
        ? `Hay un sobrante de ${money(value.difference)}. Podrás continuar únicamente después de autorizarlo.`
        : (value.error||`Hay un faltante de ${money(Math.abs(value.difference))}. No se puede continuar.`);
    apply.disabled=!value.canApply;
    return value;
  }
  ...
  $('#clearCashSheet',sheet).onclick=()=>{resetAuthorization();inputs.forEach(input=>{input.value=0;input.classList.remove('input-error');});paint();focusAndSelect(inputs[0]);};
  $('#cancelCashSurplus',sheet).onclick=()=>{
    resetAuthorization();
    const status=$('#cashSheetStatus',sheet);
    status.className='lock-alert warn';
    status.textContent='Sobrante no autorizado. El desglose permanece abierto.';
    focusAndSelect(inputs[0]);
  };
  authorize.onclick=()=>{
    const value=paint();
    const key=cashBreakdownSurplusAuthorizationKeyV945(value);
    if(!value.hasSurplus||!pendingAuthorizationKey||key!==pendingAuthorizationKey){
      resetAuthorization();
      return;
    }
    if(!canAuthorizeCashSurplusV9452()){
      resetAuthorization();
      const status=$('#cashSheetStatus',sheet);
      status.className='lock-alert bad';
      status.textContent='Solo Gerencia o Administración puede autorizar un sobrante de efectivo.';
      return;
    }
    finish(value,true);
  };
  apply.onclick=()=>{
    ...
    if(value.hasSurplus){
      if(!canAuthorizeCashSurplusV9452()){
        const status=$('#cashSheetStatus',sheet);
        status.className='lock-alert bad';
        status.textContent=`Hay un sobrante de ${money(value.difference)}. Solo Gerencia o Administración puede autorizarlo.`;
        return;
      }
      pendingAuthorizationKey=cashBreakdownSurplusAuthorizationKeyV945(value);
      $('#cashSurplusExpected',sheet).textContent=money(value.expected);
      $('#cashSurplusCounted',sheet).textContent=money(value.counted);
      $('#cashSurplusAmount',sheet).textContent=money(value.difference);
      $('#cashSurplusActor',sheet).textContent=`Autoriza: ${currentWorkerName()} · ${state.profile?.rol||'Sin rol'}`;
...
function openLiquidacionOrdenModal(o){
  ...
  function paint(){
    ...
    const surplus=cashBreakdown.hasSurplus
      ? `Efectivo esperado ${money(cashBreakdown.expected)} · físico contado ${money(cashBreakdown.counted)} · sobrante ${money(cashBreakdown.difference)}. ${cashBreakdown.surplusAuthorized?'Autorizado para esta recepción.':'Abre el desglose y solicita autorización antes de recibir.'}`
      : '';
  ...
  btn.onclick=async()=>{
    ...
    try{
      const rows=x.result==='Devuelto parcial'
        ...
        : null;
      const result=await receiveOrderCxcV945(o,{result:x.result,rows,cash:x.cash,method:method.value,physicalCash:x.cashBreakdown.expected,cashBreakdown:x.cashBreakdown.breakdown,cashAdjustment:x.cashBreakdown.adjustment,authorizeSurplus,receivedBy:by,note:notes.value||''});
      m.remove(); await refreshVisibleModuleV9384(); render();
...
function openCloseBatchLiquidationModal(deliveryName,g){
  ...
  function paint(){
    ...
    else if(cashBreakdown.hasSurplus){ msg.className='lock-alert warn'; msg.innerHTML=`Efectivo esperado <b>${money(cashBreakdown.expected)}</b> · físico contado <b>${money(cashBreakdown.counted)}</b> · sobrante <b>${money(cashBreakdown.difference)}</b>. ${cashBreakdown.surplusAuthorized?'Autorizado para este cierre.':'Abre el desglose y solicita autorización antes de cerrar.'}`; }
  ...
  $('#saveBatchClose',m).onclick=async()=>{
    ...
    try{
      const response=await receiveBatchCxcV9392R2(g,{items,receivedBy:recibido_por,note:obs,cashBreakdown:summaryNow.cashBreakdown.breakdown,cashAdjustment:summaryNow.cashBreakdown.adjustment,authorizeSurplus});
      ...
      m.remove();
      printLiquidationReceipt(deliveryName,g.code,finalOrders,{efectivo_recibido:summaryNow.cashTotal,recibido_por,observacion:obs,cash_count:{desglose:summaryNow.cashBreakdown.breakdown,efectivo_esperado:summaryNow.cashBreakdown.expected,efectivo_contado:summaryNow.cashBreakdown.counted,ajuste_fraccion:summaryNow.cashBreakdown.adjustment,diferencia:summaryNow.cashBreakdown.difference,sobrante_autorizado:authorizeSurplus,sobrante_monto:authorizeSurplus?summaryNow.cashBreakdown.difference:0,sobrante_autorizado_por_nombre:authorizeSurplus?currentWorkerName():null}},true);
      await refreshVisibleModuleV9384(); render();
...
function cashBreakdownReceiptHtml(count){
  ...
  const authorizationName=count.sobrante_autorizado_por_nombre||(count.sobrante_autorizado_por?usuarioNameFromId(count.sobrante_autorizado_por):'');
  const surplus=Number(count.sobrante_monto||0)>0.009
    ? `<p><b>Sobrante autorizado:</b> ${money(count.sobrante_monto)}${authorizationName?` · <b>Autorizado por:</b> ${esc(authorizationName)}`:''}${count.sobrante_autorizado_en?` · <b>Fecha:</b> ${businessDateTime(count.sobrante_autorizado_en)}`:''}</p>`
    : (Number(count.diferencia||0)>0.009?`<p><b>Sobrante pendiente de autorización:</b> ${money(count.diferencia)}</p>`:'');
  return `<div class="box"><b>Conteo físico de efectivo</b>${detail}<p><b>Esperado:</b> ${money(count.efectivo_esperado||0)} · <b>Físico contado:</b> ${money(count.efectivo_contado||0)} · <b>Ajuste de fracción:</b> ${money(count.ajuste_fraccion||0)} · <b>Conciliado:</b> ${money(Number(count.efectivo_contado||0)+Number(count.ajuste_fraccion||0))} · <b>Diferencia:</b> ${money(count.diferencia||0)}</p>${surplus}</div>`;
}
...
function printHistorySummary(deliveryName,rows,from='',to=''){
  const htmlRows=rows.map(l=>{ const items=historyLotItems(l), delivered=historyDeliveryDate(l,items), deliveredLabel=normalizeOperationalDate(delivered)?operationalDateLabel(delivered):(delivered?businessDateTime(delivered):'—'); return `<tr><td>${esc(historyVisualCode(l))}</td><td>${esc(l.delivery_nombre||deliveryName||'')}</td><td>${deliveredLabel}</td><td>${l.fecha_liquidacion?businessDateTime(l.fecha_liquidacion):'—'}</td><td>${Number(l.cantidad_ordenes||items.length)}</td><td>${money(l.total_facturado||0)}</td><td>${money(l.efectivo_recibido||l.efectivo_reportado||0)}</td><td>${money(l.credito_pendiente||0)}</td><td>${money(l.diferencia||0)}</td></tr>`; }).join('');
  const range=(from||to)?`<br><b>Rango:</b> ${esc(historyRangeLabel(from,to))}`:'';
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>Historial liquidaciones</title><style>body{font-family:Arial,sans-serif;font-size:12px;padding:20px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:6px;text-align:left}th{background:#f3f4f6}@media print{button{display:none}}</style></head><body>${printCompanyHeader(appCfg('recibos.tituloHistorial','Historial de liquidaciones'),'Liquidaciones cerradas')}<p><b>Delivery:</b> ${esc(deliveryName||'Todos')} · <b>Impreso:</b> ${businessDateTime(new Date())}${range}</p><table><thead><tr><th>Lote</th><th>Delivery</th><th>Fecha del lote</th><th>Liquidado</th><th>Órdenes</th><th>Total</th><th>Efectivo</th><th>Crédito</th><th>Diferencia</th></tr></thead><tbody>${htmlRows}</tbody></table>${printFooterHtml()}<button onclick="window.print()">Imprimir</button><script>setTimeout(()=>window.print(),400)<\/script></body></html>`;
  const w=window.open('','_blank','width=950,height=750'); if(!w) return alert('El navegador bloqueó la ventana de impresión.'); w.document.open(); w.document.write(html); w.document.close();
}
