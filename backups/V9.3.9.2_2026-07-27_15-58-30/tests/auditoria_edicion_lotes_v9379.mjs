...
const checks=[
  ['versión V9.3.9.0 o superior sincronizada',/^9\.3\.(?:[89]|\d{2,})\./.test(pkg.version)&&new RegExp(`V${pkg.version.replaceAll('.','\\.')} PWA`).test(main)],
  ...
  ['nota histórica no reasigna lote formal',/if\(!formalLot\) return true;[\s\S]*batchDetailRowsForCode\(code\)\.some\(d=>String\(d\.orden_id\)===String\(o\?\.id\)\)/.test(main)],
  ...
  ['SQL 38 disponible en el proyecto',/editar_composicion_lote_v9379/.test(sql)],
...
console.log('Auditoría Edición Segura de Lotes V9.3.9.0 aprobada.');
