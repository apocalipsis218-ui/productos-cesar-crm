...
async function fetchPendingOrdersV9380(orderSelect){
  ...
  for(let from=0;from<20000;from+=pageSize){
    const {data,error}=await sb.from('ordenes')
      .select(orderSelect)
      .not('estado','in','("Cobrado","Entregada en negocio","Anulado")')
  ...
  return {data:rows,error:new Error('La consulta de órdenes pendientes superó 20,000 filas.')};
}
async function loadAll(){
  ...
  const [mods,perms,ums,cats,items,pls,clientes,llamadas,productos,ordenes,ordenesPendientes,cobranza,usuarios,deliverys,empleados,pesos,entregas,pagos,historialEstados,sistemaCfg,entregaLotes,entregaLoteDetalle,entregaDocumentosHistorial,liquidacionesLotes,liquidacionLoteDetalle,casosHistorial,deliveryLotCorrections,liquidacionLotEvents,deliveryTransfers] = await Promise.all([
    ...
    safe(sb.from('ordenes').select(orderSelect).order('id',{ascending:false}).limit(500),'órdenes'),
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.8.0 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.8.0 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.8.0 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.8.0 PWA</span></div>
...
async function openTakeOrderModal(o){
  ...
  $('#confirmTake',m).onclick=async()=>{
    ...
    const {data:takenRows,error}=await sb.from('ordenes')
      .update(row)
      .eq('id',o.id)
      .is('tomado_por',null)
      .eq('estado',old)
      .select('id,codigo,estado,tomado_por,tomado_por_empleado_id,tomado_en,tomado_por_user');
    if(error) return alert(error.message);
    if(!takenRows?.length){
      await loadAll();
