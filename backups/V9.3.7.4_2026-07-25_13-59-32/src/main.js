...
<<<<<<< Updated upstream
// V9.3.7.3 · Módulo de Órdenes compacto, paginado y priorizado para operación diaria.
=======
>>>>>>> Stashed changes
...
function openMobileMoreMenu(items){
  ...
  $('[data-mobile-refresh]',overlay).onclick=async()=>{await loadAll();state.liveLastRefresh=new Date().toISOString();closeMobileMore();render();toast('Datos actualizados');};
...
async function loadAll(){
  ...
  const [mods,perms,ums,cats,items,pls,clientes,llamadas,productos,ordenes,cobranza,usuarios,deliverys,empleados,pesos,entregas,pagos,historialEstados,sistemaCfg,entregaLotes,entregaLoteDetalle,entregaDocumentosHistorial,liquidacionesLotes,liquidacionLoteDetalle,casosHistorial,deliveryLotCorrections,liquidacionLotEvents,deliveryTransfers] = await Promise.all([
    ...
    safe(sb.from('ordenes').select('*, cliente:clientes(id,codigo,negocio,contacto,telefono,sector,tipo,vendedor,estado,ultimo_pedido,credito,limite_credito), items:orden_detalle(*)').order('id',{ascending:false}).limit(500),'órdenes'),
    ...
    safe(sb.from('sistema_configuracion').select('*').in('clave',['control_peso','empresa','menu','alertas','impresion','recibos','respaldo','atajos','facturacion','seguridad','incentivos','whatsapp']),'configuración sistema'),
  ...
  if(sistemaCfg?.data?.length){
    ...
    const cfgPatch={}; ['empresa','menu','alertas','impresion','recibos','respaldo','atajos','facturacion','seguridad','incentivos','whatsapp'].forEach(k=>{ if(byKey[k]!==undefined && byKey[k]!==null) cfgPatch[k]=byKey[k]; });
...
<<<<<<< Updated upstream
  root.innerHTML = `<div class="shell${sidebarCollapsed?' sidebar-collapsed':''}"><aside id="appSidebar" class="sidebar" aria-hidden="${sidebarCollapsed?'true':'false'}"><div class="brand"><div class="logo">${esc(appCfg('empresa.logoTexto','PC'))}</div><div><h1>${esc(appCfg('empresa.nombre','Sistema Productos César'))}</h1><p>V9.3.7.3 PWA · ${esc(appCfg('empresa.subtitulo','CRM · Despacho · CXC'))}</p></div></div><nav class="nav">${renderSideNav(visibleNav)}</nav><div class="side-card"><b>V9.3.7.3 PWA</b><br>Órdenes compactas · Delivery consultivo · Liquidación centralizada.</div></aside><button id="sidebarToggle" class="sidebar-toggle" type="button" data-collapsed="${sidebarCollapsed?'1':'0'}" aria-controls="appSidebar" aria-expanded="${sidebarCollapsed?'false':'true'}" aria-label="${sidebarCollapsed?'Mostrar menú lateral':'Ocultar menú lateral'}" title="${sidebarCollapsed?'Mostrar menú lateral':'Ocultar menú lateral'}"><span aria-hidden="true">${sidebarCollapsed?'›':'‹'}</span></button><main class="main"><div class="top"><div class="mobile-brand-mini"><span>${esc(appCfg('empresa.logoTexto','PC'))}</span></div><div class="title"><h2>${titleOf(state.page)}</h2><p>${subtitleOf(state.page)}</p></div><div class="user-pill"><span title="${esc(currentUserEmail())}">${esc(currentWorkerName())} · ${esc(state.profile?.rol||'')}</span><button id="myAccessBtn" class="gray" aria-label="Mi acceso">Mi acceso</button><button id="refreshBtn" aria-label="Actualizar">Actualizar</button><button id="logoutBtn" class="dark" aria-label="Salir">Salir</button></div></div>${state.errors.length?`<div class="error"><b>Avisos:</b><br>${state.errors.map(esc).join('<br>')}<br><small>Si falta una tabla o no ves clientes, ejecuta el SQL V5.5.1 de mapeo de roles.</small></div>`:''}${liveStatusHtml()}<div id="content"></div></main><nav class="bottom-nav">${renderBottomNav(visibleNav)}</nav></div>`;
=======
  root.innerHTML = `<div class="shell${sidebarCollapsed?' sidebar-collapsed':''}"><aside id="appSidebar" class="sidebar" aria-hidden="${sidebarCollapsed?'true':'false'}"><div class="brand"><div class="logo">${esc(appCfg('empresa.logoTexto','PC'))}</div><div><h1>${esc(appCfg('empresa.nombre','Sistema Productos César'))}</h1><p>V9.3.7.2 PWA · ${esc(appCfg('empresa.subtitulo','CRM · Despacho · CXC'))}</p></div></div><nav class="nav">${renderSideNav(visibleNav)}</nav><div class="side-card"><b>V9.3.7.2 PWA</b><br>Delivery consultivo · Liquidación centralizada · cierre sin duplicados.</div></aside><button id="sidebarToggle" class="sidebar-toggle" type="button" data-collapsed="${sidebarCollapsed?'1':'0'}" aria-controls="appSidebar" aria-expanded="${sidebarCollapsed?'false':'true'}" aria-label="${sidebarCollapsed?'Mostrar menú lateral':'Ocultar menú lateral'}" title="${sidebarCollapsed?'Mostrar menú lateral':'Ocultar menú lateral'}"><span aria-hidden="true">${sidebarCollapsed?'›':'‹'}</span></button><main class="main"><div class="top"><div class="mobile-brand-mini"><span>${esc(appCfg('empresa.logoTexto','PC'))}</span></div><div class="title"><h2>${titleOf(state.page)}</h2><p>${subtitleOf(state.page)}</p></div><div class="user-pill"><span title="${esc(currentUserEmail())}">${esc(currentWorkerName())} · ${esc(state.profile?.rol||'')}</span><button id="myAccessBtn" class="gray" aria-label="Mi acceso">Mi acceso</button><button id="refreshBtn" aria-label="Actualizar">Actualizar</button><button id="logoutBtn" class="dark" aria-label="Salir">Salir</button></div></div>${state.errors.length?`<div class="error"><b>Avisos:</b><br>${state.errors.map(esc).join('<br>')}<br><small>Si falta una tabla o no ves clientes, ejecuta el SQL V5.5.1 de mapeo de roles.</small></div>`:''}${liveStatusHtml()}<div id="content"></div></main><nav class="bottom-nav">${renderBottomNav(visibleNav)}</nav></div>`;
>>>>>>> Stashed changes
...
function matchOrder(o,q){ q=norm(q); if(!q) return true; return norm([orderClientName(o),orderClientPhone(o),orderClientSector(o),o?.codigo,o?.factura_no,o?.estado,orderDeliveryMode(o)].join(' ')).includes(q); }
...
function canonicalOrderStates(){ return ['Programada','Pedido recibido','En preparación','Lista para facturar','Impresa para facturar','Facturada','Lista para retiro','Entregada en negocio','Asignada a delivery','En ruta','Cobrado','Entregado a crédito','No entregado','Devuelto parcial','Anulado']; }
...
function orderCustomerBadge(o){ return isInternalSale(o)?'<span class="badge internal-sale-badge">Venta interna</span>':''; }
...
function renderOrdenes(c){
  const estados=['Programada','Pedido recibido','En preparación','Lista para facturar','Impresa para facturar','Facturada','Lista para retiro','Entregada en negocio','Asignada a delivery','En ruta','Cobrado','Entregado a crédito','No entregado','Anulado'];
  ...
  const rows=(q ? base.filter(o=>matchOrder(o,q)) : base).sort((a,b)=>{
    ...
    return late(b)-late(a) || priority(b)-priority(a) || Number(b.id||0)-Number(a.id||0);
  });
  const pageSize=25;
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
<<<<<<< Updated upstream
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7.3 PWA</span></div>
=======
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7.2 PWA</span></div>
>>>>>>> Stashed changes
...
function bindReportCommercialV9215(c,ctx){
  ...
  $('#refreshReportBtn',c).onclick=async()=>{await loadAll();renderReportes(c);toast('Reportes actualizados');};
...
<<<<<<< Updated upstream
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7.3 PWA</span></div>
=======
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7.2 PWA</span></div>
>>>>>>> Stashed changes
  <div class="grid2">
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="hint" style="margin-bottom:8px">Variables permitidas: {contacto}, {negocio}, {encabezado}, {codigo_orden}, {fecha_orden}, {fecha_despacho}, {hora_despacho}, {detalle_sin_precio}, {observacion_cliente}.</div>
...
<<<<<<< Updated upstream
  c.innerHTML=`<div class="panel-head"><div><h3>Usuarios, empleados y permisos</h3><p>V9.3.7.3 PWA: cada acceso personal se vincula con el empleado real; las cuentas compartidas se identifican como estaciones.</p></div><div class="actions"><button class="btn gray" id="refreshUsers">Actualizar</button><button class="btn gray" id="authGuide">Guía crear login</button></div></div>
=======
  c.innerHTML=`<div class="panel-head"><div><h3>Usuarios, empleados y permisos</h3><p>V9.3.7.2 PWA: cada acceso personal se vincula con el empleado real; las cuentas compartidas se identifican como estaciones.</p></div><div class="actions"><button class="btn gray" id="refreshUsers">Actualizar</button><button class="btn gray" id="authGuide">Guía crear login</button></div></div>
>>>>>>> Stashed changes
  ...
  $('#refreshUsers').onclick=async()=>{ await loadAll(); state.configTab='usuarios'; renderConfig($('#content')); toast('Usuarios y empleados actualizados'); };
...
function openUserPerms(u){
  ...
  $('#saveUserPerms',m).onclick=async()=>{
    ...
    try{
      ...
      m.remove(); await loadAll(); state.configTab='usuarios'; renderConfig($('#content')); toast('Usuario vinculado y permisos actualizados');
    }catch(e){
      console.error(e);
...
<<<<<<< Updated upstream
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7.3 PWA</span></div>
=======
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7.2 PWA</span></div>
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7.3 PWA</span></div>
=======
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7.2 PWA</span></div>
>>>>>>> Stashed changes
...
function orderWhatsAppPhone(c){
  const num=onlyNum(c?.telefono||'');
  if(!num) return '';
...
function buildOrderWhatsAppMessage(o,kind='confirmacion',templateOverride=null){
  ...
  const map={
    contacto:o?.cliente?.contacto||o?.cliente?.negocio||'cliente',
    negocio:o?.cliente?.negocio||'',
    encabezado:header,
...
async function openOrderWhatsApp(o,kind='confirmacion'){
  ...
  if(!isCommercialNormalOrder(o)) return alert('La confirmación automática está disponible para pedidos normales.');
  if(!(o.items||[]).length) return alert('La orden no tiene productos para enviar.');
  const phone=orderWhatsAppPhone(o.cliente);
...
function maybeOfferOrderWhatsApp(o,kind='confirmacion'){
  const cfg=orderWhatsAppConfig();
  if(cfg.ofrecerAlGuardar===false || !o || o.estado==='Anulado' || !isCommercialNormalOrder(o)) return;
  if(!orderWhatsAppPhone(o.cliente) || !(o.items||[]).length) return;
  showOrderWhatsAppPrompt(o,kind);
}
function orderWhatsAppManualButton(o){
  const cfg=orderWhatsAppConfig();
  if(cfg.botonManual===false || !o || o.estado==='Anulado' || !isCommercialNormalOrder(o) || !orderWhatsAppPhone(o.cliente)) return '';
...
function openOrderForm(o=null, client=null, extra={}){
  ...
  const body=`<div class="form order-form-pro order-form-r10"><input type="hidden" id="ordTotal" value="${currentTotal}">
    <div class="order-section order-client-section-r10"><div class="section-title">1. Cliente e identificación</div><div class="grid2 order-customer-mode-grid"><div class="field"><label>Tipo de cliente</label><select id="ordCustomerType"><option value="Registrado" ${initialCustomerType==='Registrado'?'selected':''}>Cliente registrado</option><option value="Venta interna" ${orderCustomerType(o)==='Venta interna'?'selected':''}>Venta interna / mostrador</option></select></div><div class="field"><label>Modalidad de entrega</label><select id="ordDeliveryMode"><option value="Delivery" ${initialDeliveryMode==='Delivery'?'selected':''}>Delivery</option><option value="Retiro en negocio" ${initialDeliveryMode==='Retiro en negocio'?'selected':''}>Retiro en negocio</option><option value="No aplica" ${initialDeliveryMode==='No aplica'?'selected':''}>No aplica</option></select></div></div><div id="registeredClientBlock" class="field relative"><label>Cliente registrado</label><input id="ordClientText" autocomplete="off" data-lpignore="true" data-form-type="other" value="${esc(client?client.codigo+' · '+client.negocio:(o?.cliente?o.cliente.codigo+' · '+o.cliente.negocio:''))}" placeholder="Buscar nombre del cliente..."><input type="hidden" id="ordClientId" value="${client?.id||o?.cliente_id||''}"><div id="ordSuggest" class="suggest" style="display:none"></div></div><div id="internalClientBlock" class="grid2" style="display:none"><div class="field"><label>Nombre del comprador *</label><input id="ordInternalName" maxlength="120" value="${esc(isInternalSale(o)?orderClientName(o):'')}" placeholder="Nombre obligatorio"></div><div class="field"><label>Teléfono opcional</label><input id="ordInternalPhone" maxlength="30" value="${esc(isInternalSale(o)?orderClientPhone(o):'')}" placeholder="Opcional"></div></div><div id="customerFlowNote" class="weight-alert info"></div></div>
  ...
  const syncCustomerModeUi=()=>{
    ...
    const registered=$('#registeredClientBlock',m), internal=$('#internalClientBlock',m), note=$('#customerFlowNote',m);
    const internalSale=type==='Venta interna';
    const incident=String($('#ordTipoOrden',m)?.value||'').includes('Incidente');
    if(registered) registered.style.display=internalSale?'none':'block';
    if(internal) internal.style.display=internalSale?'grid':'none';
    ...
    if(note) note.innerHTML=internalSale?'<strong>Venta interna</strong>El nombre es obligatorio, no crea ficha de cliente, queda al contado y se retira en el negocio.':'<strong>Cliente registrado</strong>Selecciona la modalidad: Delivery o Retiro en negocio.';
  };
  ...
  $('#saveOrder',m).onclick=async()=>{
    ...
    if(customerType==='Venta interna' && !internalName) return alert('Es obligatorio escribir el nombre del comprador.');
    ...
    const customerPhone=customerType==='Registrado'?String(cl?.telefono||'').trim():String($('#ordInternalPhone',m)?.value||'').trim();
    const customerSector=customerType==='Registrado'?String(cl?.sector||'').trim():'Mostrador';
...
<<<<<<< Updated upstream
  return {version:'V9.3.7.3 PWA',codigo_lote:lote,delivery_nombre:deliveryName,fecha_entrega:originalDate,validado_por:validatedBy,empresa:deliveryRouteSnapshotCompany(),items,totales:{cantidad_ordenes:items.length,total_facturado:items.reduce((a,x)=>a+x.monto_factura,0),peso_esperado:items.reduce((a,x)=>a+x.peso_esperado,0),peso_entregado:items.reduce((a,x)=>a+x.peso_entregado,0)}};
=======
  return {version:'V9.3.7.2 PWA',codigo_lote:lote,delivery_nombre:deliveryName,fecha_entrega:originalDate,validado_por:validatedBy,empresa:deliveryRouteSnapshotCompany(),items,totales:{cantidad_ordenes:items.length,total_facturado:items.reduce((a,x)=>a+x.monto_factura,0),peso_esperado:items.reduce((a,x)=>a+x.peso_esperado,0),peso_entregado:items.reduce((a,x)=>a+x.peso_entregado,0)}};
>>>>>>> Stashed changes
}
...
function openOrderTransferModal(lot,o){
  ...
  $('#saveOrderTransfer',m).onclick=async()=>{
    ...
    m.remove();await loadAll();renderValidacion($('#content'));toast(`Pedido transferido a ${responsible.name} · ${data?.codigo_lote_destino||''}`);
  };
}
...
function bindValidationBatch(container,orders){
  ...
  $('#createDeliveryBatch',container).onclick=async()=>{
    ...
    await loadAll(); render(); toast(`Lote ${data?.codigo_lote||lote} creado para ${responsible.name}`);
  };
  updateBatchSummary(container,orders);
}
...
async function receiveOrderCxcV937(o,payload){
  ...
  const {data,error}=await sb.rpc('recibir_orden_cxc_v937',{
...
async function receiveBatchCxcV937(g,payload){
  if(!state.v937SchemaOk) throw new Error('Primero ejecuta el SQL 30 de la V9.3.7 en Supabase.');
  const lot=batchRecordByCode(g.code);
  if(!lot?.id) throw new Error('No se encontró el lote formal. Actualiza la pantalla.');
  const {data,error}=await sb.rpc('recibir_lote_cxc_v937',{
...
async function consolidateDuplicateLiquidations(code){
  ...
  if(error) return alert(error.message);
  await loadAll(); render(); toast(data?.duplicados_eliminados?`Duplicados consolidados: ${data.duplicados_eliminados}`:'El lote ya estaba consolidado.');
}
...
async function openTakeOrderModal(o){
  ...
  const body=`<div class="form"><div class="client-card" style="grid-template-columns:1fr"><div><div class="client-title">${esc(o.codigo)} · ${esc(orderClientName(o))}</div><div class="client-sub">${orderItemsText(o,10)}</div></div></div><div class="queue-box"><div><b>Tu cola actual: ${currentCount}/3</b><div class="limit">La regla evita que un despachador tome más de 3 pedidos a la vez.</div></div></div>${workerField}<div class="lock-alert ok">Al tomar el pedido quedará visible para todos, pero bloqueado como: <b>En preparación por tu usuario</b>.</div><button class="btn" id="confirmTake">Tomar pedido y preparar</button></div>`;
...
async function printPreparationTicket(o){
  ...
  </style></head><body><div class="center"><h2>PRODUCTOS CÉSAR</h2><div class="b">ORDEN DE PREPARACIÓN</div><div>${esc(o.codigo||'')}</div></div>${pickupAlert}<div class="line"></div><div>Fecha orden: ${shortDate(o.fecha)}</div><div>Fecha despacho: ${shortDate(dispatchDateOf(o))} ${o.hora_despacho?esc(String(o.hora_despacho).slice(0,5)):''}</div><div>Impreso: ${now.toLocaleString('es-DO')}</div>${isFutureDispatch(o)?'<div class="b">NO DESPACHAR HOY</div>':''}<div class="line"></div><div class="b">CLIENTE</div><div>${esc(orderClientName(o))}</div><div>Tel: ${esc(orderClientPhone(o))}</div><div>Sector: ${esc(orderClientSector(o))}</div><div class="line"></div><div>Tomado por: ${esc(workerDisplayName(o.tomado_por||o.preparado_por)||'________________')}</div><div>Hora tomada: ${o.tomado_en?new Date(o.tomado_en).toLocaleString('es-DO'):'________________'}</div><div class="line"></div><div class="b">DETALLE SIN PRECIOS</div><table>${lines}</table><div class="line"></div>${o.notas?`<div>Notas: ${esc(o.notas)}</div>`:''}${o.nota_programacion?`<div>Programación: ${esc(o.nota_programacion)}</div>`:''}<div class="sign">Peso final</div><div class="sign">Paquetes</div><div class="sign">Firma despacho</div><button onclick="window.print()">Imprimir</button><script>setTimeout(()=>window.print(),400)<\/script></body></html>`;
...
function openValidacionModal(o){
  ...
  save.onclick=async()=>{
    ...
    m.remove(); await loadAll(); render(); toast(`Orden asignada a ${del} en ${data?.codigo_lote||lote}`);
  };
  focusEl(valMonto);
}
...
function openLiquidacionOrdenModal(o){
  ...
  <div id="liqSummary" class="lock-alert ok"></div><div class="grid2"><div class="field"><label>Recibido por</label><select id="liqBy">${employeeOptions('CXC',currentWorkerName())}</select>${manualInput('liqByManual')}</div><div class="field"><label>Observación</label><textarea id="liqNotas" placeholder="Opcional"></textarea></div></div><button class="btn" id="saveLiq">Recibir cliente</button></div>`;
  const m=openModal('Recibir cliente en Liquidación / CXC',body,'El cliente desaparece de Pendientes al confirmar. El último cliente cierra automáticamente el lote.');
  ...
  function calc(){ try{return {...calculateCentralReceipt(total,res.value,Number(cash.value||0)),error:''};}catch(err){return {total,result:res.value,cash:Number(cash.value||0),pending:0,error:err.message||String(err)};} }
  ...
  res.onchange=()=>{ if(res.value==='Cobrado') cash.value=total; if(res.value==='No entregado') cash.value=0; if(res.value==='Entregado a crédito'&&Number(cash.value||0)>=total) cash.value=0; paint(); };
  ...
  btn.onclick=async()=>{
    ...
    try{
      const result=await receiveOrderCxcV937(o,{result:x.result,cash:x.cash,method:method.value,receivedBy:by,note:notes.value||''});
      m.remove(); await loadAll(); render();
      toast(result?.lote_cerrado?`Cliente recibido. Lote ${result.codigo_lote} cerrado.`:`Cliente recibido. Quedan ${result?.pendientes??0} pendiente(s) en el lote.`);
...
function openCloseBatchLiquidationModal(deliveryName,g){
  ...
  $('#saveBatchClose',m).onclick=async()=>{
    ...
    try{
      const response=await receiveBatchCxcV937(g,{items,receivedBy:recibido_por,note:obs});
      ...
      await loadAll(); render(); toast(`Lote ${response?.codigo_lote||g.code} recibido y cerrado sin duplicados.`);
...
function openEmpleadoForm(e={}){
  ...
  const body=`<div class="form"><div class="grid2"><div class="field"><label>Nombre</label><input id="empNombre" value="${esc(e.nombre||'')}"></div><div class="field"><label>Área / rol operativo</label><select id="empArea">${areas.map(a=>`<option ${a===e.area?'selected':''}>${a}</option>`).join('')}</select><div class="hint">El área sugiere el rol del usuario, pero no cambia sus permisos automáticamente.</div></div></div><div class="field"><label>Estado</label><select id="empActivo"><option value="true" ${e.activo!==false?'selected':''}>Activo</option><option value="false" ${e.activo===false?'selected':''}>Inactivo</option></select></div>${linkedUser?`<div class="success"><b>Acceso vinculado:</b> ${esc(linkedUser.correo||linkedUser.nombre||linkedUser.id)}. Al cambiar el nombre se sincronizará con Usuarios. Al desactivar al empleado también se desactivará su acceso.</div>`:''}<div class="field"><label>Observaciones</label><textarea id="empObs">${esc(e.observaciones||'')}</textarea></div><button class="btn" id="saveEmp">Guardar empleado</button></div>`;
  ...
  $('#saveEmp',m).onclick=async()=>{
    ...
    m.remove(); await loadAll(); render(); toast(linkedUser?'Empleado y usuario sincronizados':'Empleado guardado');
  };
