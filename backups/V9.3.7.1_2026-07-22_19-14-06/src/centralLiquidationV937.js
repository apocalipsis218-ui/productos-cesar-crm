...
export function buildPendingDeliveryPanel(lots = [], details = [], orders = [], now = new Date()){
  ...
  lots.filter(l=>!['cerrado','revertido'].includes(String(l?.estado||'').toLowerCase())).forEach(lot=>{
    ...
    rows.push({
      ...
      delivery: lot.delivery_nombre || pending[0]?.delivery_nombre || 'Sin delivery',
  ...
  rows.forEach(row=>{
    if(!byDelivery.has(row.delivery)) byDelivery.set(row.delivery,{delivery:row.delivery,lots:0,pendingClients:0,pendingAmount:0,oldestMinutes:0,oldestAt:'',partialLots:0,lotRows:[]});
    const item=byDelivery.get(row.delivery);
    item.lots += 1;
