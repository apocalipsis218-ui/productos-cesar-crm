...
// V9.3.8.4 · Carga incremental y rendimiento optimizado para tabletas.
...
async function cancelOrder(o, opts={}){
  ...
  if(advanced){
    ...
    if(r.error){ alert(r.error.message+'\n\nVerifica que aplicaste el SQL 41 de la actualización anterior.'); return false; }
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.8.4 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.8.4 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.8.4 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.8.4 PWA</span></div>
...
function openCloseBatchLiquidationModal(deliveryName,g){
  ...
  $('#saveBatchClose',m).onclick=async()=>{
    ...
    try{
      ...
      await refreshVisibleModuleV9384(); render(); toast(`Lote ${response?.codigo_lote||g.code} recibido y cerrado sin duplicados.`);
