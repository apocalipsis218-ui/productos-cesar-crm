...
// V9.3.8.2 · Estados, permisos y campos críticos protegidos.
...
async function annulOrder(o, reason=''){
  ...
  const r=await sb.rpc('cambiar_estado_orden_v9382',{p_orden_id:o.id,p_estado_esperado:old,p_estado_nuevo:'Anulado',p_cambios:{notas},p_comentario:nota,p_modulo:'ordenes'});
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.8.2 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.8.2 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.8.2 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.8.2 PWA</span></div>
...
async function setOrderState(o, estado, extra={}){
  ...
  if(error){ alert('No se pudo cambiar el estado: '+error.message+'\n\nVerifica que aplicaste el SQL 40 de V9.3.8.2.'); return false; }
  await loadAll(); render(); toast('Orden actualizada: '+estado); return true;
}
...
async function openTakeOrderModal(o){
  ...
  $('#confirmTake',m).onclick=async()=>{
    ...
    const row={tomado_por:nombre,tomado_por_empleado_id:employee?.id||null,tomado_en:new Date().toISOString(),tomado_por_user:state.user.id,preparado_por:null,preparado_en:null,liberado_por:null,liberado_en:null,motivo_liberacion:null};
    const {data:takenRows,error}=await sb.rpc('cambiar_estado_orden_v9382',{p_orden_id:o.id,p_estado_esperado:old,p_estado_nuevo:'En preparación',p_cambios:row,p_comentario:`Orden tomada por ${nombre}${isStationAccount()?` desde la estación ${currentUserEmail()}`:''}`,p_modulo:'carniceria'});
    if(error) return alert(error.message);
    if(!takenRows?.length){
      await loadAll();
...
function openPreparacionModal(o){
  ...
  const updateDetailsAndOrder=async(final=false)=>{
    ...
    const saved=await sb.rpc('guardar_preparacion_v9381',{p_orden_id:o.id,p_lineas:prepLines,p_cabecera:workBase,p_final:!!final});
    ...
    toast(final?'Orden preparada y enviada a Facturación.':'Avance guardado. La orden aún no está marcada como preparada.');
  };
