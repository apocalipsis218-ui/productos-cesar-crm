...
$backupDir = Join-Path $projectDir "backups\V9.3.8.2_$timestamp"
$excludedNames = @('APLICAR_V9382.ps1', 'LEEME_V9.3.8.2.txt')

Write-Host ''
Write-Host 'Productos Cesar CRM - V9.3.8.2 AUDITADA' -ForegroundColor Cyan
Write-Host 'Estados, permisos y campos criticos protegidos'
Write-Host ''
...
Write-Host ''
Write-Host 'V9.3.8.2 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'IMPORTANTE: ejecuta en Supabase el SQL 40 antes de probar el flujo.' -ForegroundColor Yellow
Write-Host 'supabase\sql\40_actualizacion_v9382_estados_protegidos.sql' -ForegroundColor Cyan
