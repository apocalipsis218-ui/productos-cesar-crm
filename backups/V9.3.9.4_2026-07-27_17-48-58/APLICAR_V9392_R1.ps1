...
$backupDir = Join-Path $projectDir "backups\V9.3.9.2_R1_$timestamp"
$excludedNames = @('LEEME_V9.3.9.2_R1.txt')

Write-Host 'Productos Cesar CRM - V9.3.9.2 R1 AUDITADA' -ForegroundColor Cyan
Write-Host 'Devuelto parcial visible desde Recibir lote, con detalle obligatorio por articulos'
...
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\45_actualizacion_v9392_devolucion_parcial_articulos.sql'))) {
  throw 'Paquete incompleto: falta el SQL 45 de la V9.3.9.2.'
}
...
Write-Host ''
Write-Host 'V9.3.9.2 R1 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Esta revision no agrega SQL nuevo. Conserva aplicado el SQL 45.' -ForegroundColor Yellow
