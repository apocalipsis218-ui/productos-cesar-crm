...
function loadHistoryUi(){
  const def={delivery:{},liquidacion:{}};
  try{
    const raw=JSON.parse(localStorage.getItem(HISTORY_UI_KEY)||'{}');
    return {delivery:{...(raw.delivery||{})},liquidacion:{...(raw.liquidacion||{})}};
  }catch(e){ return def; }
}
...
function setHistoryOpen(scope,key,open){
  state.historyUi=state.historyUi||{delivery:{},liquidacion:{}};
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.5 PWA</span></div>
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.5 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.5 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.5 PWA</span></div>
...
function renderValidationBatchRow(o){
  ...
  return `<div class="batch-row ${orderTypeClass(o)}" data-batch-row="${o.id}" data-amount="${amount}" data-expected="${Number(ref.value||0)}" data-reqpeso="${req?'1':'0'}">
    ...
    <div class="batch-amount"><span>Factura</span><b>${amount>0?money(amount):'RD$ —'}</b></div>
...
function openLiquidacionOrdenModal(o){
  ...
  const body=`<div class="form"><div class="client-card" style="grid-template-columns:1fr"><div><div class="client-title">${esc(orderClientName(o))}</div><div class="client-sub">${esc(o.codigo)} · Delivery: ${esc(o.delivery_nombre||'')} · total factura ${money(total)}</div></div></div>
...
function printLiquidationReceipt(deliveryName,code,orders,recibo={},auto=true){
  const summary=deliveryMoneySummary(orders);
  const rows=orders.map(o=>`<tr><td>${esc(o.codigo||'')}</td><td>${esc(orderClientName(o))}</td><td>${esc(o.factura_no||'—')}</td><td>${esc(o.resultado_entrega||o.estado||'')}</td><td>${money(orderMonto(o))}</td><td>${money(o.monto_cobrado||0)}</td><td>${money(o.monto_pendiente||0)}</td></tr>`).join('');
