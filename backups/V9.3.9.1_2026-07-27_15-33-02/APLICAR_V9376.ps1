...
$backupDir = Join-Path $projectDir "backups\V9.3.7.6_$timestamp"
$excludedNames = @('APLICAR_V9376.ps1', 'LEEME_V9.3.7.6.txt')

Write-Host ''
Write-Host 'Productos Cesar CRM - V9.3.7.6 AUDITADA' -ForegroundColor Cyan
Write-Host 'Sincronizacion de pesaje al editar productos de una orden'
Write-Host ''
...
Write-Host ''
Write-Host 'V9.3.7.6 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'No requiere SQL nuevo. Ejecuta npm.cmd run dev para la prueba local.' -ForegroundColor Cyan
