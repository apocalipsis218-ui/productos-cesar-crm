...
// V9.3.7.8 · Módulo de Órdenes compacto, paginado y priorizado para operación diaria.
...
async function loadAll(){
  ...
  const [mods,perms,ums,cats,items,pls,clientes,llamadas,productos,ordenes,cobranza,usuarios,deliverys,empleados,pesos,entregas,pagos,historialEstados,sistemaCfg,entregaLotes,entregaLoteDetalle,entregaDocumentosHistorial,liquidacionesLotes,liquidacionLoteDetalle,casosHistorial,deliveryLotCorrections,liquidacionLotEvents,deliveryTransfers] = await Promise.all([
    ...
    safe(sb.from('ordenes').select('*, cliente:clientes(id,codigo,negocio,contacto,telefono,sector,direccion,referencia,tipo,vendedor,estado,ultimo_pedido,credito,limite_credito), items:orden_detalle(*)').order('id',{ascending:false}).limit(500),'órdenes'),
...
function renderOrdenes(c){
  ...
  if(view==='accion') base=base.filter(o=>o.estado!=='Anulado' && ['Pedido recibido','En preparación','Lista para facturar','Impresa para facturar','Facturada','Validada para delivery','Lista para retiro'].includes(effectiveOrderState(o)));
...
function renderConfigIncentivos(c){
  const cfg=incentiveConfig();
  c.innerHTML=`<div class="panel-head"><div><h3>Incentivos / Productividad</h3><p>Configura cómo se calcula el incentivo mensual. Delivery y despacho se miden por cliente; el lote queda como opción alternativa.</p></div><span class="badge info">V9.3.7.8 PWA</span></div>
...
function exportAuditExceptions(rows){
  const data=rows.map(x=>({'Fecha':x.creado_en?new Date(x.creado_en).toLocaleString('es-DO'):'','Gravedad':x.gravedad,'Estado revisión':x.estado_revision,'Tipo':x.tipo_evento,'Módulo':x.modulo,'Usuario':x.usuario_nombre,'Empleado':x.empleado_nombre||'','Estación':x.cuenta_estacion||'','Orden':x.orden_codigo||'','Cliente':x.cliente_nombre||'','Lote':x.lote_codigo||'','Esperado':x.valor_esperado??'','Registrado':x.valor_registrado??'','Diferencia':x.diferencia??'','Unidad':x.unidad||'','Motivo':x.motivo,'Nota administrativa':x.nota_administrativa||''}));
...
function openAuditExceptionReview(row){
  ...
  $('#saveAuditReview',m).onclick=async()=>{
    ...
    m.remove(); await loadAll(); render(); toast('Revisión administrativa guardada');
  };
}
function renderAuditoria(c){
  ...
  ${state.auditExceptionsSchemaOk?'':`<div class="error"><b>Auditoría V9.3.7.8 pendiente:</b> ejecuta el SQL 36 para activar el registro privado de excepciones.</div>`}
...
function renderConfigPlantillas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>WhatsApp</h3><p>Configura la confirmación de órdenes y las plantillas generales para clientes.</p></div><span class="badge info">V9.3.7.8 PWA</span></div>
  <div class="grid2">
    <div class="card">
...
function renderConfigGeneral(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Centro de configuración</h3><p>Todo lo que cambies aquí alimenta los módulos operativos del sistema.</p></div><span class="badge info">V9.3.7.8 PWA</span></div>
...
function renderConfigAlertas(c){
  ...
  c.innerHTML=`<div class="panel-head"><div><h3>Alertas, horario y SLA</h3><p>Configura el tiempo laborable real, los límites por etapa y las reglas del análisis operativo V9.2.15.</p></div><span class="badge info">V9.3.7.8 PWA</span></div>
...
function openOrderForm(o=null, client=null, extra={}){
  ...
  $('#saveOrder',m).onclick=async()=>{
    ...
    if(o){
      const deleted=await sb.from('orden_detalle').delete().eq('orden_id',o.id).select('id');
      if(deleted.error) return alert('No se pudo actualizar el detalle de la orden: '+deleted.error.message+'\n\nDebes aplicar el SQL de seguridad incluido en V9.3.7.8.');
      if((deleted.data||[]).length!==(o.items||[]).length) return alert('Supabase no autorizó eliminar todas las líneas anteriores. La orden no fue modificada.\n\nAplica el SQL 35 de V9.3.7.8 y vuelve a intentarlo.');
      ...
      if(existing.data){
        ...
        if(r.error) return alert(r.error.message);
        const deleted=await sb.from('orden_detalle').delete().eq('orden_id',orderId).select('id');
        if(deleted.error) return alert('No se pudo actualizar el detalle de la orden: '+deleted.error.message);
      }else{
    ...
    if(o&&composition.changed){
      ...
      await logOrderState(o,o.estado,'Pedido recibido',`Composición modificada después del pesaje${removedNames?`; productos eliminados: ${removedNames}`:''}. Se invalidaron los pesos posteriores y la orden requiere confirmación y nuevo pesaje en Carnicería.`);
    }
...
async function openTakeOrderModal(o){
  ...
  $('#confirmTake',m).onclick=async()=>{
    ...
    const row={estado:'En preparación',tomado_por:nombre,tomado_por_empleado_id:employee?.id||null,tomado_en:new Date().toISOString(),tomado_por_user:state.user.id,preparado_por:null,preparado_en:null,liberado_por:null,liberado_en:null,motivo_liberacion:null};
    const {error}=await sb.from('ordenes').update(row).eq('id',o.id);
    if(error) return alert(error.message);
    await logOrderState(fresh||o,old,'En preparación',`Orden tomada por ${nombre}${isStationAccount()?` desde la estación ${currentUserEmail()}`:''}`);
...
async function printPreparationTicket(o){
  ...
  </style></head><body><div class="center"><h2>PRODUCTOS CÉSAR</h2><div class="b">ORDEN DE PREPARACIÓN</div><div>${esc(o.codigo||'')}</div></div>${pickupAlert}<div class="line"></div><div>Fecha orden: ${shortDate(o.fecha)}</div><div>Fecha despacho: ${shortDate(dispatchDateOf(o))} ${o.hora_despacho?esc(String(o.hora_despacho).slice(0,5)):''}</div><div>Impreso: ${now.toLocaleString('es-DO')}</div>${isFutureDispatch(o)?'<div class="b">NO DESPACHAR HOY</div>':''}<div class="line"></div><div class="b">CLIENTE</div><div>${esc(orderClientName(o))}</div><div>Tel: ${esc(orderClientPhone(o))}</div><div>Sector: ${esc(orderClientSector(o))}</div>${occasionalPrintBlock(o)}<div class="line"></div><div>Tomado por: ${esc(workerDisplayName(o.tomado_por||o.preparado_por)||'________________')}</div><div>Hora tomada: ${o.tomado_en?new Date(o.tomado_en).toLocaleString('es-DO'):'________________'}</div><div class="line"></div><div class="b">DETALLE SIN PRECIOS</div><table>${lines}</table><div class="line"></div>${o.notas?`<div>Notas: ${esc(o.notas)}</div>`:''}${o.nota_programacion?`<div>Programación: ${esc(o.nota_programacion)}</div>`:''}<div class="sign">Peso final</div><div class="sign">Paquetes</div><div class="sign">Firma despacho</div><button onclick="window.print()">Imprimir</button><script>setTimeout(()=>window.print(),400)<\/script></body></html>`;
