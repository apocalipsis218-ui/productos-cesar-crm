...
$backupDir = Join-Path $projectDir "backups\V9.3.9.6_$timestamp"
$excludedNames = @('LEEME_V9.3.9.6.txt')

Write-Host 'Productos Cesar CRM - V9.3.9.6 AUDITADA' -ForegroundColor Cyan
Write-Host 'Cancelacion segura de pedidos No entregados retornados'

if (-not (Test-Path (Join-Path $projectDir 'package.json'))) {
  throw 'No encontre package.json en la carpeta superior. Extrae el ZIP dentro de C:\Proyectos\productos-cesar-crm.'
}
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\48_actualizacion_v9395_corregir_no_entregado_validacion.sql'))) {
  throw 'Paquete incompleto: falta el SQL 48.'
}
if (-not (Test-Path (Join-Path $hotfixDir 'supabase\sql\49_actualizacion_v9396_cancelar_no_entregado_retornado.sql'))) {
  throw 'Paquete incompleto: falta el SQL 49.'
}
...
Write-Host ''
Write-Host 'V9.3.9.6 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Ejecuta el SQL 49 en Supabase antes de repetir la anulacion.' -ForegroundColor Yellow
