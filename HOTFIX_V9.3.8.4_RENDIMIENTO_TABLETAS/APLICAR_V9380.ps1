...
$backupDir = Join-Path $projectDir "backups\V9.3.8.0_$timestamp"
$excludedNames = @('APLICAR_V9380.ps1', 'LEEME_V9.3.8.0.txt')

Write-Host ''
Write-Host 'Productos Cesar CRM - V9.3.8.0 AUDITADA' -ForegroundColor Cyan
Write-Host 'Estabilidad, concurrencia y recuperabilidad'
Write-Host ''
...
Write-Host ''
Write-Host 'V9.3.8.0 aplicada, auditada y compilada correctamente.' -ForegroundColor Green
Write-Host 'Esta actualizacion no requiere ejecutar un SQL nuevo en Supabase.' -ForegroundColor Yellow
Write-Host 'Los SQL 19 al 32 se restauraron solo para recuperabilidad del repositorio.' -ForegroundColor Cyan
